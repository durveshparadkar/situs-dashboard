 ...
 */
const numericString = (defaultValue) => z
    .string()
    .optional()
    .transform((v, ctx) => {
    if (v === undefined || v === "") {
...
const booleanString = (defaultValue) => z
    .string()
    .optional()
    .transform((v) => {
    if (v === undefined || v === "")
...
const envSchema = z.object({
    ...
    MONGO_URI: z
        ...
        .refine((v) => v.startsWith("mongodb://") || v.startsWith("mongodb+srv://"), { message: "MONGO_URI must start with mongodb:// or mongodb+srv://" }),
...
if (IS_PRODUCTION) {
    if (e.JWT_SECRET === "situs_secret" || e.JWT_SECRET.length < 64) {
        contextualErrors.push("JWT_SECRET in production should be a strong random value of 64+ characters");
    }
    if (!e.JWT_REFRESH_SECRET) {
        contextualWarnings.push("JWT_REFRESH_SECRET not set — refresh tokens will use JWT_SECRET (less secure)");
    }
    if (!e.STRIPE_SECRET_KEY) {
        contextualWarnings.push("STRIPE_SECRET_KEY not set — billing features will be disabled");
    }
    if (e.STRIPE_SECRET_KEY && !e.STRIPE_WEBHOOK_SECRET) {
        contextualErrors.push("STRIPE_SECRET_KEY is set but STRIPE_WEBHOOK_SECRET is missing — webhooks will be unverified");
    }
    if (!e.API_BASE_URL) {
        contextualWarnings.push("API_BASE_URL not set — webhook URLs and redirects may be malformed");
    }
    if (!e.FRONTEND_URL) {
        contextualWarnings.push("FRONTEND_URL not set — email links and OAuth redirects may be malformed");
    }
    if (!e.SENTRY_DSN) {
        contextualWarnings.push("SENTRY_DSN not set — production errors will not be tracked");
    }
}
const smallBusinessMonthlyPrice = e.STRIPE_PRICE_SMALL_BUSINESS_MONTHLY ?? e.STRIPE_PRICE_SMALL_MONTHLY;
const isBillingEnabled = Boolean(e.STRIPE_SECRET_KEY && e.STRIPE_WEBHOOK_SECRET);
// Cross-field rules for billing
if (isBillingEnabled) {
    const missingPrices = [];
    if (!smallBusinessMonthlyPrice)
        missingPrices.push("SMALL_BUSINESS");
    if (!e.STRIPE_PRICE_PRO_MONTHLY)
        missingPrices.push("PRO");
    if (!e.STRIPE_PRICE_ENTERPRISE_MONTHLY)
        missingPrices.push("ENTERPRISE");
    if (missingPrices.length > 0) {
        contextualWarnings.push("Stripe enabled but missing price IDs for plans: " + missingPrices.join(", "));
    }
}
// SMTP all-or-nothing
...
if (smtpSetCount > 0 && smtpSetCount < 4) {
    contextualWarnings.push("SMTP partially configured — email sending will fail. " +
...
export const env = {
    ...
    email: {
        ...
        from: e.EMAIL_FROM,
        isEnabled: Boolean(e.SMTP_HOST && e.SMTP_USER && e.SMTP_PASS && e.EMAIL_FROM),
    },
    // -----------------------------------------------------------
    // OBSERVABILITY
    // -----------------------------------------------------------
