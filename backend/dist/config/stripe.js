...
if (!STRIPE_CONFIG.secretKey) {
    if (isProduction) {
        ...
        dbLogger.error("STRIPE_SECRET_KEY not set in production. Billing features will be disabled.");
    }
    else {
        dbLogger.warn("STRIPE_SECRET_KEY not set — Stripe client will throw on use. " +
...
if (STRIPE_CONFIG.secretKey) {
    ...
    if (!isTestKey && !isLiveKey) {
        dbLogger.error("STRIPE_SECRET_KEY has invalid format. Expected sk_test_* or sk_live_*.");
    }
    if (isProduction && isTestKey) {
        dbLogger.error("STRIPE TEST KEY in PRODUCTION environment. Real payments will fail. " +
            "Verify your secrets configuration.");
    }
    if (!isProduction && isLiveKey) {
        dbLogger.warn("STRIPE LIVE KEY in non-production environment. " +
...
if (isProduction && STRIPE_CONFIG.secretKey && !STRIPE_CONFIG.webhookSecret) {
    dbLogger.error("STRIPE_SECRET_KEY is set but STRIPE_WEBHOOK_SECRET is missing. " +
...
dbLogger.info("Stripe client initialized: " +
    "apiVersion=" + STRIPE_CONFIG.apiVersion +
 ...
 */
export function buildIdempotencyKey(operation, parts) {
    const cleaned = parts
...
export function verifyWebhookSignature(payload, signature) {
    ...
    return stripe.webhooks.constructEvent(payload, sigHeader, STRIPE_CONFIG.webhookSecret);
}
/**
...
export async function getStripeHealth() {
    ...
    try {
        ...
        status.lastError = err?.message ?? "unknown";
        dbLogger.error("Stripe health check failed: " +
            (err?.message ?? "unknown"));
    }
    return status;
}
 ...
 */
export async function safeRetrieveCustomer(customerId) {
    if (!isStripeEnabled())
        return null;
    try {
        const customer = await stripe.customers.retrieve(customerId);
        if (customer.deleted) {
            dbLogger.warn("Stripe customer is deleted: id=" + customerId);
            return null;
        }
        return customer;
    }
 ...
 */
export async function safeRetrieveSubscription(subscriptionId) {
    if (!isStripeEnabled())
        return null;
    try {
 ...
 *   );
 */
export async function withStripeTimeout(promise, ms) {
    let timer;
    const timeoutPromise = new Promise((_resolve, reject) => {
        timer = setTimeout(() => reject(new Error("Stripe operation timed out after " + ms + "ms")), ms);
    });
    try {
