...
function attachEventListeners() {
    ...
    conn.on("connected", () => {
        dbLogger.info("MongoDB connection event: connected " +
            "host=" + (conn.host ?? "unknown") +
            " db=" + (conn.name ?? "unknown"));
    });
    conn.on("disconnected", () => {
        dbLogger.warn("MongoDB connection event: disconnected " +
            "host=" + (conn.host ?? "unknown"));
    });
    conn.on("reconnected", () => {
        dbLogger.info("MongoDB connection event: reconnected " +
            "host=" + (conn.host ?? "unknown"));
    });
    conn.on("error", (err) => {
        lastConnectionError = err;
        dbLogger.error("MongoDB connection event: error " +
...
// CONNECT WITH RETRY
// ============================================================
async function attemptConnection(mongoUri, attempt) {
    dbLogger.info("MongoDB connecting: attempt=" + attempt + "/" + DB_CONFIG.maxRetries +
    ...
    connectionPromise = (async () => {
        ...
        for (let attempt = 1; attempt <= DB_CONFIG.maxRetries; attempt++) {
            ...
            try {
                ...
                dbLogger.error("MongoDB connection attempt failed: " +
                    "attempt=" + attempt + "/" + DB_CONFIG.maxRetries +
                    " error=" + (lastError.message ?? "unknown"));
                if (attempt < DB_CONFIG.maxRetries) {
                    const delay = getRetryDelay(attempt);
                    dbLogger.info("MongoDB retrying in " + delay + "ms");
                    await sleep(delay);
                }
            }
        }
        ...
        isConnecting = false;
        const finalError = new Error("MongoDB connection failed after " + DB_CONFIG.maxRetries +
...
export async function disconnectDB() {
    ...
    try {
        ...
        dbLogger.error("MongoDB disconnect error: " +
            (err?.message ?? "unknown"));
        throw err;
    }
}
...
export function registerShutdownHandlers() {
    ...
    const shutdown = async (signal) => {
        ...
        try {
            ...
            dbLogger.error("Graceful shutdown error: " +
                (err?.message ?? "unknown"));
            process.exit(1);
        }
    };
    ...
    process.on("uncaughtException", (err) => {
        dbLogger.error("Uncaught exception: " + (err?.message ?? "unknown"));
        void shutdown("uncaughtException");
    });
    process.on("unhandledRejection", (reason) => {
        dbLogger.error("Unhandled promise rejection: " +
