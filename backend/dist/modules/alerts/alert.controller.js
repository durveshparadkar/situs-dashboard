...
const listAlertsSchema = z
    ...
    fromDate: z.string().optional().refine((v) => !v || !isNaN(new Date(v).getTime()), { message: "fromDate must be a valid ISO date" }),
    toDate: z.string().optional().refine((v) => !v || !isNaN(new Date(v).getTime()), { message: "toDate must be a valid ISO date" }),
...
const bulkActionSchema = z
    ...
    alertIds: z.array(objectIdSchema)
        ...
        .max(ALERT_CONFIG.bulkOperation.maxIds, "Cannot operate on more than " + ALERT_CONFIG.bulkOperation.maxIds + " alerts at once"),
})
    .strict();
...
class AlertController {
    ...
    // -----------------------------------------------------------
    getAlerts = asyncHandler(async (req, res, _next) => {
        ...
        const limit = getNumber(validated.limit, ALERT_CONFIG.pagination.defaultLimit, { min: 1, max: ALERT_CONFIG.pagination.maxLimit });
        const options = {
            page,
            limit,
        ...
        if (validated.status)
            options.status = validated.status;
        if (validated.severity)
            options.severity = validated.severity;
        if (validated.type)
            options.type = validated.type.toUpperCase();
        if (validated.isRead !== undefined)
            options.isRead = validated.isRead === "true";
        if (validated.fromDate)
            options.fromDate = new Date(validated.fromDate);
        if (validated.toDate)
            options.toDate = new Date(validated.toDate);
    ...
    // -----------------------------------------------------------
    getAlertById = asyncHandler(async (req, res, _next) => {
        const actor = requireAuth(req);
        const id = requireObjectId(req);
        const ServiceAny = AlertService;
        if (typeof ServiceAny.getAlertById !== "function") {
            throw new AppError("Alert lookup by ID not supported", HttpStatus.INTERNAL, "NOT_IMPLEMENTED");
        }
    ...
    // -----------------------------------------------------------
    getUnreadCount = asyncHandler(async (req, res, _next) => {
        const actor = requireAuth(req);
    ...
    // -----------------------------------------------------------
    getAlertStats = asyncHandler(async (req, res, _next) => {
        const actor = requireAuth(req);
        const ServiceAny = AlertService;
    ...
    // -----------------------------------------------------------
    markAsRead = asyncHandler(async (req, res, _next) => {
        const actor = requireAuth(req);
        const id = requireObjectId(req);
        const ServiceAny = AlertService;
        ...
        dbLogger.info("Alert marked read: id=" + id +
    ...
    // -----------------------------------------------------------
    markAllAsRead = asyncHandler(async (req, res, _next) => {
        const actor = requireAuth(req);
        const ServiceAny = AlertService;
        const result = await ServiceAny.markAllAsRead(actor.organizationId, actor.userId);
        const modifiedCount = result?.modifiedCount ?? 0;
        dbLogger.info("Alerts mark-all-read: org=" + actor.organizationId +
            " user=" + actor.userId +
    ...
    // -----------------------------------------------------------
    bulkMarkAsRead = asyncHandler(async (req, res, _next) => {
        const actor = requireAuth(req);
        const validated = runSchema(bulkActionSchema, req.body);
        const ServiceAny = AlertService;
        let modifiedCount = 0;
        if (typeof ServiceAny.bulkMarkAsRead === "function") {
            const result = await ServiceAny.bulkMarkAsRead(validated.alertIds, actor.organizationId, actor.userId);
        ...
        dbLogger.info("Alerts bulk read: org=" + actor.organizationId +
            " user=" + actor.userId +
    ...
    // -----------------------------------------------------------
    resolveAlert = asyncHandler(async (req, res, _next) => {
        const actor = requireAuth(req);
        const id = requireObjectId(req);
        const ServiceAny = AlertService;
        ...
        dbLogger.info("Alert resolved: id=" + id +
            " org=" + actor.organizationId +
            " user=" + actor.userId);
        res.status(HttpStatus.OK).json({
            success: true,
    ...
    // -----------------------------------------------------------
    bulkResolve = asyncHandler(async (req, res, _next) => {
        const actor = requireAuth(req);
        const validated = runSchema(bulkActionSchema, req.body);
        const ServiceAny = AlertService;
        let modifiedCount = 0;
        if (typeof ServiceAny.bulkResolve === "function") {
            const result = await ServiceAny.bulkResolve(validated.alertIds, actor.organizationId, actor.userId);
            modifiedCount = result?.modifiedCount ?? 0;
        }
        else {
            for (const alertId of validated.alertIds) {
                const result = await ServiceAny.resolveAlert(alertId, actor.organizationId, actor.userId);
                if (result)
                    modifiedCount++;
            }
        }
        dbLogger.warn("Alerts bulk resolved: org=" + actor.organizationId +
            " user=" + actor.userId +
            " requested=" + validated.alertIds.length +
            " modified=" + modifiedCount);
        res.status(HttpStatus.OK).json({
    ...
    // DELETE /alerts/:id
    // -----------------------------------------------------------
    deleteAlert = asyncHandler(async (req, res, _next) => {
        const actor = requireAuth(req);
        const id = requireObjectId(req);
        const ServiceAny = AlertService;
        ...
        dbLogger.warn("Alert deleted: id=" + id +
            " org=" + actor.organizationId +
            " user=" + actor.userId);
        res.status(HttpStatus.OK).json({
            success: true,
    ...
    // -----------------------------------------------------------
    bulkDelete = asyncHandler(async (req, res, _next) => {
        const actor = requireAuth(req);
        const validated = runSchema(bulkActionSchema, req.body);
        const ServiceAny = AlertService;
        let deletedCount = 0;
        if (typeof ServiceAny.bulkDelete === "function") {
            const result = await ServiceAny.bulkDelete(validated.alertIds, actor.organizationId, actor.userId);
            deletedCount = result?.deletedCount ?? 0;
        }
        else {
            for (const alertId of validated.alertIds) {
                const result = await ServiceAny.deleteAlert(alertId, actor.organizationId, actor.userId);
                if (result)
                    deletedCount++;
            }
        }
        dbLogger.warn("Alerts bulk deleted: org=" + actor.organizationId +
            " user=" + actor.userId +
            " requested=" + validated.alertIds.length +
