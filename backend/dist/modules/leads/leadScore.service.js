...
function calculatePriority(score) {
    if (score >= SCORE_CONFIG.priority.critical)
        return "critical";
    if (score >= SCORE_CONFIG.priority.high)
        return "high";
    if (score >= SCORE_CONFIG.priority.medium)
        return "medium";
    return "low";
}
...
class LeadScoreService {
    async calculateLeadScore(leadId) {
        ...
        let isStale = false;
        if (diffDays <= 1)
            score += SCORE_CONFIG.recency.day1;
        else if (diffDays <= 3)
            score += SCORE_CONFIG.recency.day3;
        else if (diffDays <= 7)
            score += SCORE_CONFIG.recency.day7;
        else {
        ...
        ====================================================== */
        const shouldUpdate = lead.leadScore !== score ||
    ...
    ====================================================== */
    async triggerAlerts(lead, score, isStale) {
        // 🟢 Hot Lead
        if (score >= 80) {
...
//# sourceMappingURL=leadScore.service.js.map