...
===================================================== */
router.post("/demo-request", demoRequestLimiter, async (req, res, next) => {
    try {
        if (!SITUS_ORG_ID || !SITUS_OWNER_USER_ID) {
            console.error("Demo request misconfigured: SITUS_DEMO_ORG_ID / SITUS_DEMO_OWNER_ID env vars not set");
            res.status(500).json({
                success: false,
        ...
        if (company)
            notificationPayload.company = company;
        if (message)
...
//# sourceMappingURL=publicDemo.routes.js.map