 ...
 */
router.get("/summary", SUMMARY_LIMIT, authorize("READ_FORECAST"), asyncHandler((req, res, next) => ForecastController.getForecastSummary(req, res, next)));
/**
 ...
 */
router.get("/breakdown", BREAKDOWN_LIMIT, authorize("READ_FORECAST"), asyncHandler((req, res, next) => ForecastController.getForecastBreakdown(req, res, next)));
/**
 ...
 */
router.get("/accuracy", ACCURACY_LIMIT, requireManager, authorize("READ_FORECAST"), asyncHandler((req, res, next) => ForecastController.getForecastAccuracy(req, res, next)));
/**
 * @route   POST /forecast/refresh
 * @desc    Force recalculation, bypassing cache (manager-only, expensive)
 * @access  Manager+ + WRITE_FORECAST
 */
router.post("/refresh", REFRESH_LIMIT, requireManager, authorize("MANAGE_FORECAST"), asyncHandler((req, res, next) => ForecastController.refreshForecast(req, res, next)));
/**
 * @route   GET /forecast
 ...
 */
router.get("/", FORECAST_LIMIT, authorize("READ_FORECAST"), asyncHandler((req, res, next) => ForecastController.getForecast(req, res, next)));
