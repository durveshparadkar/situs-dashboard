...
class TeamService {
    ...
    /* ================= GET ONE ================= */
    async getById(teamId, currentUser) {
    ...
    /* ================= UPDATE ================= */
    async update(teamId, data, currentUser) {
        const session = await mongoose.startSession();
        try {
            let updatedTeam;
            await session.withTransaction(async () => {
                ...
                if (data.members) {
                    ...
                    await Membership.deleteMany({ teamId: team._id }, { session });
                    // 🔥 Add new memberships
                    await Membership.insertMany(newMembers.map((userId) => ({
                        userId,
                        teamId: team._id,
                        role: "USER",
                    })), { session });
    ...
    /* ================= DELETE (SOFT) ================= */
    async remove(teamId, currentUser) {
        const team = await Team.findOne({
            _id: toObjectId(teamId),
            organizationId: toObjectId(currentUser.organizationId),
        });
    ...
    /* ================= ADD MEMBER ================= */
    async addMember(teamId, userId, currentUser) {
        const team = await Team.findOne({
            _id: toObjectId(teamId),
            organizationId: toObjectId(currentUser.organizationId),
        });
