...
const requireManager = (req, res, next) => {
    ...
    if (!MANAGER_ROLES.includes(role)) {
        dbLogger.warn(`Team mutation denied: user=${req.user?.id ?? "anon"} role=${role}`);
        res.status(403).json({
            success: false,
            error: {
                code: "FORBIDDEN",
...
const requireAdmin = (req, res, next) => {
    ...
    if (!ADMIN_ROLES.includes(role)) {
        dbLogger.warn(`Team admin action denied: user=${req.user?.id ?? "anon"} role=${role}`);
        res.status(403).json({
            success: false,
            error: {
                code: "FORBIDDEN",
...
const checkTeamLimitMw = checkTeamLimit;
 ...
 * @rateLimit 10/min
 */
router.post("/", CREATE_LIMIT, requireActiveBillingMw, authorizeMw("CREATE_TEAM"), requireManager, checkTeamLimitMw, teamController.create);
/**
 * @route   GET /teams
 ...
 */
router.get("/", READ_LIMIT, authorizeMw("READ_TEAM"), teamController.getAll);
/* =====================================================
 ...
 * @access  Authenticated + READ_TEAM
 */
router.get("/:id", READ_LIMIT, authorizeMw("READ_TEAM"), validateObjectId("id"), teamController.getOne);
/**
 ...
 */
router.patch("/:id", WRITE_LIMIT, requireActiveBillingMw, authorizeMw("UPDATE_TEAM"), requireManager, validateObjectId("id"), teamController.update);
/**
 * @route   DELETE /teams/:id
 * @desc    Soft-delete a team
 * @access  Manager+ + DELETE_TEAM + active billing
 */
router.delete("/:id", WRITE_LIMIT, requireActiveBillingMw, authorizeMw("DELETE_TEAM"), requireManager, validateObjectId("id"), teamController.remove);
/* =====================================================
   TEAM MEMBER ROUTES
===================================================== */
/**
 ...
 * @access  Authenticated + READ_TEAM
 */
router.get("/:id/members", READ_LIMIT, authorizeMw("READ_TEAM"), validateObjectId("id"), teamController.getMembers);
/**
 ...
 * @body    { userId: string }
 */
router.post("/:id/members", MEMBER_LIMIT, requireActiveBillingMw, authorizeMw("UPDATE_TEAM"), requireManager, validateObjectId("id"), teamController.addMember);
/**
 ...
 * @rateLimit 5/min
 */
router.post("/:id/members/bulk-add", BULK_LIMIT, requireActiveBillingMw, authorizeMw("UPDATE_TEAM"), requireManager, validateObjectId("id"), teamController.bulkAddMembers);
/**
 * @route   DELETE /teams/:id/members/:userId
 * @desc    Remove a member from a team (REST style)
 * @access  Manager+ + UPDATE_TEAM + active billing
 */
router.delete("/:id/members/:userId", MEMBER_LIMIT, requireActiveBillingMw, authorizeMw("UPDATE_TEAM"), requireManager, validateObjectId("id"), validateObjectId("userId"), teamController.removeMember);
/**
 ...
 * @body    { userId: string }
 */
router.delete("/:id/members", MEMBER_LIMIT, requireActiveBillingMw, authorizeMw("UPDATE_TEAM"), requireManager, validateObjectId("id"), teamController.removeMember);
/* =====================================================
   ADMIN-ONLY ROUTES
===================================================== */
/**
 ...
 */
router.post("/:id/transfer-manager", TRANSFER_LIMIT, requireActiveBillingMw, authorizeMw("UPDATE_TEAM"), requireAdmin, validateObjectId("id"), teamController.transferManager);
