...
const VALID_RATINGS = ["helpful", "not_helpful", "irrelevant"];
...
function requirePrivileged(actor) {
    if (!INSIGHT_CONFIG.privilegedRoles.includes(actor.role)) {
...
function requireObjectId(value, fieldName) {
    ...
    if (!id) {
        throw new AppError(`${fieldName} is required`, HttpStatus.BAD_REQUEST, "MISSING_ID");
    }
    if (!mongoose.Types.ObjectId.isValid(id)) {
        throw new AppError(`Invalid ${fieldName} format`, HttpStatus.BAD_REQUEST, "INVALID_ID");
    }
    return id;
}
...
function parseEnumSingle(value, allowed) {
...
class InsightController {
    ...
    async createInsight(req, res, next) {
        try {
            ...
            if (missing.length > 0) {
                throw new ValidationError(missing.map((field) => ({ field, message: "Required field missing" })));
            }
            /* ── Validate target type against allowlist ── */
            if (!VALID_TARGET_TYPES.includes(body.target.type)) {
                throw new AppError(`Invalid target.type. Must be one of: ${VALID_TARGET_TYPES.join(", ")}`, HttpStatus.BAD_REQUEST, "INVALID_TARGET_TYPE");
            }
            ...
            dbLogger.info(`Insight created: type=${input.type} target=${target.type}/${target.id} ` +
    ...
    ===================================================== */
    async listInsights(req, res, next) {
        try {
            ...
            const limit = Math.min(Math.max(parseInt10(req.query.limit, INSIGHT_CONFIG.pagination.defaultLimit), 1), INSIGHT_CONFIG.pagination.maxLimit);
            /* ── Sort with allowlist ── */
            const sortByRaw = paramAsString(req.query.sortBy);
            const sortBy = sortByRaw && VALID_SORT_FIELDS.includes(sortByRaw)
                ? sortByRaw
                : "generatedAt";
    ...
    ===================================================== */
    async getMyInsights(req, res, next) {
        try {
            const { userId, organizationId } = requireAuth(req);
            const limit = Math.min(Math.max(parseInt10(req.query.limit, 50), 1), INSIGHT_CONFIG.caps.maxMyInsightsLimit);
            const minSeverity = parseEnumSingle(req.query.minSeverity, VALID_SEVERITIES);
            const insights = await insightService.getInsightsForRep(userId, organizationId, {
                limit,
    ...
    async getInsightById(req, res, next) {
        try {
            ...
            if (!insight) {
                throw new AppError("Insight not found", HttpStatus.NOT_FOUND, "INSIGHT_NOT_FOUND");
            }
    ...
    async getByTarget(req, res, next) {
        try {
            ...
            if (!targetTypeRaw ||
                !VALID_TARGET_TYPES.includes(targetTypeRaw.toLowerCase())) {
                throw new AppError(`Invalid target type. Must be one of: ${VALID_TARGET_TYPES.join(", ")}`, HttpStatus.BAD_REQUEST, "INVALID_TARGET_TYPE");
            }
            if (!targetId) {
                throw new AppError("targetId is required", HttpStatus.BAD_REQUEST, "MISSING_TARGET_ID");
            }
    ...
    async markActed(req, res, next) {
        try {
            ...
            dbLogger.info(`Insight acted: id=${id} user=${userId} org=${organizationId}`);
            res.status(HttpStatus.OK).json({
                success: true,
                data: insight,
    ...
    async dismissInsight(req, res, next) {
        try {
            const { userId, organizationId } = requireAuth(req);
            const id = requireObjectId(req.params.id, "Insight ID");
            const reason = req.body?.reason !== undefined && req.body.reason !== null
                ? clampString(req.body.reason, INSIGHT_CONFIG.caps.reason)
                : undefined;
            const insight = await insightService.dismiss(id, userId, organizationId, reason);
            if (!insight) {
                throw new AppError("Insight not found", HttpStatus.NOT_FOUND, "INSIGHT_NOT_FOUND");
            }
            dbLogger.info(`Insight dismissed: id=${id} user=${userId} reason=${reason ?? "none"}`);
            res.status(HttpStatus.OK).json({
                success: true,
                data: insight,
    ...
    ===================================================== */
    async snoozeInsight(req, res, next) {
        try {
            const { userId, organizationId } = requireAuth(req);
            const id = requireObjectId(req.params.id, "Insight ID");
            const until = parseDate(req.body?.until);
            if (!until) {
                throw new AppError("Valid 'until' date (ISO format) is required", HttpStatus.BAD_REQUEST, "INVALID_SNOOZE_DATE");
            }
            /* Reject snoozing to a past date */
            if (until.getTime() <= Date.now()) {
                throw new AppError("Snooze 'until' must be in the future", HttpStatus.BAD_REQUEST, "INVALID_SNOOZE_DATE");
            }
            ...
            if (until.getTime() - Date.now() > maxSnoozeMs) {
                throw new AppError("Cannot snooze more than 90 days", HttpStatus.BAD_REQUEST, "SNOOZE_TOO_LONG");
            }
            const insight = await insightService.snooze(id, userId, organizationId, until);
            if (!insight) {
                throw new AppError("Insight not found", HttpStatus.NOT_FOUND, "INSIGHT_NOT_FOUND");
            }
            res.status(HttpStatus.OK).json({
    ...
    async submitFeedback(req, res, next) {
        try {
            ...
            if (!VALID_RATINGS.includes(rating)) {
                throw new AppError(`Invalid rating. Must be one of: ${VALID_RATINGS.join(", ")}`, HttpStatus.BAD_REQUEST, "INVALID_RATING");
            }
            const feedback = {
                rating: rating,
                ...(comment !== undefined &&
            ...
            const insight = await insightService.submitFeedback(id, userId, organizationId, feedback);
            if (!insight) {
                throw new AppError("Insight not found", HttpStatus.NOT_FOUND, "INSIGHT_NOT_FOUND");
            }
            dbLogger.info(`Insight feedback: id=${id} rating=${rating} user=${userId}`);
            res.status(HttpStatus.OK).json({
                success: true,
                data: insight,
    ...
    ===================================================== */
    async archiveInsight(req, res, next) {
        try {
            const { userId, organizationId } = requireAuth(req);
            const id = requireObjectId(req.params.id, "Insight ID");
            const insight = await insightService.archiveInsight(id, userId, organizationId);
            if (!insight) {
                throw new AppError("Insight not found", HttpStatus.NOT_FOUND, "INSIGHT_NOT_FOUND");
            }
            res.status(HttpStatus.OK).json({
    ...
    async bulkCreate(req, res, next) {
        try {
            ...
            if (!Array.isArray(inputs) || inputs.length === 0) {
                throw new AppError("Request body must contain a non-empty 'insights' array", HttpStatus.BAD_REQUEST, "INVALID_PAYLOAD");
            }
            if (inputs.length > INSIGHT_CONFIG.caps.maxBulkSize) {
                throw new AppError(`Cannot bulk-create more than ${INSIGHT_CONFIG.caps.maxBulkSize} insights at once`, HttpStatus.BAD_REQUEST, "TOO_MANY_INSIGHTS");
            }
            ...
            dbLogger.info(`Insight bulk: org=${organizationId} user=${userId} ` +
                `requested=${inputs.length} created=${result.created} ` +
                `updated=${result.updated} failed=${result.failed}`);
            res.status(HttpStatus.OK).json({
                success: true,
                data: result,
                message: `Bulk processed: ${result.created} created, ` +
    ...
    async expireStale(req, res, next) {
        try {
            ...
            dbLogger.warn(`Insights expired: org=${organizationId} count=${count} user=${userId}`);
            res.status(HttpStatus.OK).json({
                success: true,
