...
export const createPipeline = async (data) => {
    ...
    try {
        ...
        await session.withTransaction(async () => {
            ...
            if (data.isDefault) {
                await Pipeline.updateMany({ organizationId: orgId, isDefault: true }, { $set: { isDefault: false } }, { session });
            }
...
===================================================== */
export const updatePipeline = async (pipelineId, data, organizationId) => {
    const session = await mongoose.startSession();
    try {
        let updated;
        await session.withTransaction(async () => {
            ...
            if (data.stages) {
                pipeline.stages = normalizeStages(data.stages);
            }
            if (data.isDefault) {
...
===================================================== */
export const deletePipeline = async (pipelineId, organizationId) => {
    const pipeline = await Pipeline.findOne({
...
===================================================== */
export const getDefaultPipeline = async (organizationId) => {
...
===================================================== */
export const getPipelinesByOrganization = async (organizationId, page = 1, limit = 10) => {
    const orgId = toObjectId(organizationId);
