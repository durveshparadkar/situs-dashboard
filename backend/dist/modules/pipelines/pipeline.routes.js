...
const requireManager = (req, res, next) => {
    ...
    if (!MANAGER_ROLES.includes(role)) {
        dbLogger.warn(`Pipeline mutation denied: user=${req.user?.id ?? "anon"} role=${role}`);
        res.status(403).json({
            success: false,
            error: {
                code: "FORBIDDEN",
 ...
 */
router.get("/default", READ_LIMIT, pipelineController.getDefault);
/**
 * @route   GET /pipelines
 ...
 */
router.get("/", READ_LIMIT, pipelineController.getAll);
/**
 ...
 * @rateLimit 10/min
 */
router.post("/", CREATE_LIMIT, requireManager, pipelineController.create);
/**
 * @route   GET /pipelines/:id
 * @desc    Get a single pipeline by ID
 * @access  Authenticated
 */
router.get("/:id", READ_LIMIT, validateObjectId("id"), pipelineController.getById);
/**
 * @route   PATCH /pipelines/:id
 * @desc    Update a pipeline's name, stages, or default flag
 * @access  Manager+
 */
router.patch("/:id", WRITE_LIMIT, requireManager, validateObjectId("id"), pipelineController.update);
/**
 * @route   DELETE /pipelines/:id
 * @desc    Delete a pipeline (non-default only)
 * @access  Manager+
 */
router.delete("/:id", WRITE_LIMIT, requireManager, validateObjectId("id"), pipelineController.remove);
/**
 * @route   POST /pipelines/:id/set-default
 * @desc    Promote a pipeline to the org's default
 * @access  Manager+
 */
router.post("/:id/set-default", WRITE_LIMIT, requireManager, validateObjectId("id"), pipelineController.setDefault);
/**
 ...
 */
router.post("/:id/duplicate", DUPLICATE_LIMIT, requireManager, validateObjectId("id"), pipelineController.duplicate);
