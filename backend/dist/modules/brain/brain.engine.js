...
===================================================== */
function buildRecommendedActions(context, signals, priority) {
    ...
    return Array.from(actionMap.values()).sort((a, b) => PRIORITY_RANK[a.priority] - PRIORITY_RANK[b.priority]);
}
/* =====================================================
   CONFIDENCE CALCULATION
   Reflects how certain the brain is of its output.
===================================================== */
function calculateConfidence(score, signals) {
    let confidence = BRAIN_CONFIG.baseConfidence;
    // High/critical signals add certainty (clearer evidence)
    const strongSignals = signals.filter(s => s.severity === "high" || s.severity === "critical").length;
...
===================================================== */
export function runBrain(context) {
    ...
    let signals;
    try {
        signals = detectSignals(context, score);
    }
    catch (err) {
    ...
    const topSignal = signals.length
        ? [...signals].sort((a, b) => PRIORITY_RANK[a.severity] - PRIORITY_RANK[b.severity])[0]?.type
        : undefined;
    /* ── 7. Assemble decision ── */
    const decision = {
        score,
...
//# sourceMappingURL=brain.engine.js.map