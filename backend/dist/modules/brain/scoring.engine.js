...
===================================================== */
function activityFactor(context) {
...
function inactivityFactor(context) {
...
function dealValueFactor(context) {
...
function stageProbabilityFactor(context) {
...
function fastMovementFactor(context) {
    const cfg = SCORING_CONFIG.fastMovement;
    if (!context.stageChangedRecently)
        return null;
    if (context.daysSinceLastActivity > cfg.inactivityCap)
        return null;
    return {
...
function leadScoreBlendFactor(context) {
...
function riskScoreFactor(context) {
...
function highValueNoReplyFactor(context) {
    const cfg = SCORING_CONFIG.highValueNoReply;
    const value = context.dealValue ?? 0;
    const days = context.daysSinceLastActivity;
    if (value < cfg.valueThreshold)
        return null;
    if (days < cfg.inactivityThreshold)
        return null;
    return {
 ...
 */
export function calculateScore(context) {
 ...
 */
export function calculateScoreWithBreakdown(context) {
    /* ── Validation ── */
    if (!context) {
        return {
