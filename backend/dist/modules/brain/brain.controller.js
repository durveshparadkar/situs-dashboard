...
class BrainController {
    ...
    ===================================================== */
    analyzeLead = asyncHandler(async (req, res, _next) => {
        const userId = getUserId(req);
        const organizationId = getOrgId(req);
        const leadId = requireObjectId(req.params.leadId, "leadId");
        /* Optional flags */
        const explainability = parseBool(req.query.explain, BRAIN_CONFIG.defaultExplainability);
        const includeSignals = parseBool(req.query.includeSignals, BRAIN_CONFIG.defaultIncludeSignals);
        dbLogger.info(`Brain analyze: lead=${leadId} user=${userId} org=${organizationId} ` +
        ...
        const decision = await withTimeout(Promise.resolve(svc.analyzeLead(leadId, {
            organizationId,
            explainability,
            includeSignals,
            requestedBy: userId,
        })), BRAIN_CONFIG.maxAnalyzeTimeoutMs, "Brain analysis");
        if (!decision) {
            throw ApiError.notFound("Lead not found or could not be analyzed");
        }
        const durationMs = Date.now() - startedAt;
        dbLogger.info(`Brain analyze complete: lead=${leadId} durationMs=${durationMs}`);
        /* Slow-analysis warning so ops can investigate */
        if (durationMs > 5_000) {
            dbLogger.warn(`Slow brain analysis: lead=${leadId} durationMs=${durationMs}`);
        }
        res.status(200).json({
            success: true,
            data: decision,
    ...
    ===================================================== */
    refreshAnalysis = asyncHandler(async (req, res, _next) => {
        const userId = getUserId(req);
        const organizationId = getOrgId(req);
        const leadId = requireObjectId(req.params.leadId, "leadId");
        const svc = brainService;
        const fn = typeof svc.refreshAnalysis === "function"
        ...
        dbLogger.info(`Brain refresh: lead=${leadId} user=${userId}`);
        const startedAt = Date.now();
        const decision = await withTimeout(Promise.resolve(fn()), BRAIN_CONFIG.maxAnalyzeTimeoutMs, "Brain refresh");
        if (!decision) {
    ...
    ===================================================== */
    analyzeBatch = asyncHandler(async (req, res, _next) => {
        const userId = getUserId(req);
        const organizationId = getOrgId(req);
        const leadIds = req.body?.leadIds;
        if (!Array.isArray(leadIds) || leadIds.length === 0) {
            throw ApiError.badRequest("Request body must contain a non-empty 'leadIds' array");
        }
        if (leadIds.length > BRAIN_CONFIG.maxBatchSize) {
            throw ApiError.badRequest(`Cannot analyze more than ${BRAIN_CONFIG.maxBatchSize} leads at once`);
        }
        /* Validate every ID before any work — fail-fast on bad input */
        const validatedIds = leadIds.map((id, idx) => {
            if (typeof id !== "string" || !mongoose.Types.ObjectId.isValid(id)) {
                throw ApiError.badRequest(`Invalid leadId at index ${idx}`);
            }
            return id;
        });
        const svc = brainService;
        dbLogger.info(`Brain analyzeBatch: count=${validatedIds.length} user=${userId} org=${organizationId}`);
        const startedAt = Date.now();
        let result;
        if (typeof svc.analyzeBatch === "function") {
            /* Service supports native batch — preferred */
            result = await withTimeout(Promise.resolve(svc.analyzeBatch(validatedIds, {
                organizationId,
                requestedBy: userId,
            })), BRAIN_CONFIG.maxBatchTimeoutMs, "Brain batch analysis");
        }
            ...
            const settled = await Promise.allSettled(validatedIds.map((id) => svc.analyzeLead(id, { organizationId, requestedBy: userId })));
        ...
        const durationMs = Date.now() - startedAt;
        dbLogger.info(`Brain analyzeBatch complete: count=${validatedIds.length} durationMs=${durationMs}`);
        res.status(200).json({
            success: true,
            data: result,
            meta: {
    ...
    ===================================================== */
    getDecision = asyncHandler(async (req, res, _next) => {
        const userId = getUserId(req);
        const organizationId = getOrgId(req);
        const leadId = requireObjectId(req.params.leadId, "leadId");
        const svc = brainService;
        const fn = typeof svc.getDecision === "function"
            ? svc.getDecision
        ...
        dbLogger.info(`Brain decision fetched: lead=${leadId} user=${userId}`);
        res.status(200).json({
            success: true,
            data: decision,
        });
    ...
    ===================================================== */
    getDecisionHistory = asyncHandler(async (req, res, _next) => {
        const userId = getUserId(req);
        const organizationId = getOrgId(req);
        const leadId = requireObjectId(req.params.leadId, "leadId");
        ...
        dbLogger.info(`Brain history fetched: lead=${leadId} count=${Array.isArray(history) ? history.length : 0} user=${userId}`);
        res.status(200).json({
            success: true,
            data: history,
            meta: {
    ...
    ===================================================== */
    submitFeedback = asyncHandler(async (req, res, _next) => {
        const userId = getUserId(req);
        const organizationId = getOrgId(req);
        const leadId = requireObjectId(req.params.leadId, "leadId");
        ...
        if (!["accurate", "inaccurate", "partially_accurate"].includes(rating)) {
            throw ApiError.badRequest("rating must be one of: accurate, inaccurate, partially_accurate");
        }
        ...
        dbLogger.info(`Brain feedback: lead=${leadId} decision=${decisionId} ` +
    ...
    ===================================================== */
    health = asyncHandler(async (req, res, _next) => {
        const userId = getUserId(req);
        const svc = brainService;
        const startedAt = Date.now();
        let isHealthy = true;
        let details = { status: "unknown" };
        ...
        const durationMs = Date.now() - startedAt;
        dbLogger.info(`Brain health check: healthy=${isHealthy} durationMs=${durationMs} user=${userId}`);
