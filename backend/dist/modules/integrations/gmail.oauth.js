...
const GMAIL_CALLBACK_URL = process.env.GMAIL_CALLBACK_URL ||
    "https://api.situsrevenue.com/api/integrations/gmail/callback";
if (!GOOGLE_CLIENT_ID || !GOOGLE_CLIENT_SECRET) {
    console.warn("⚠️  GOOGLE_CLIENT_ID / GOOGLE_CLIENT_SECRET not set — Gmail integration will fail until these env vars are configured.");
}
/**
...
export function createGmailOAuthClient() {
    return new google.auth.OAuth2(GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, GMAIL_CALLBACK_URL);
}
const GMAIL_SCOPES = [
...
export async function exchangeGmailAuthCode(code) {
    ...
    if (!tokens.access_token || !tokens.refresh_token || !tokens.expiry_date) {
        throw new Error("Google did not return the expected tokens — refresh_token is missing. " +
...
//# sourceMappingURL=gmail.oauth.js.map