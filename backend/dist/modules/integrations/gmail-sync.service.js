...
async function syncMailbox(connectionId) {
    // Re-fetch with sensitive fields (accessToken/refreshToken are
    // select: false by default on the schema)
    const connection = await GmailConnection.findById(connectionId).select("+accessToken +refreshToken");
    ...
    try {
        ...
        for (const msg of messages) {
            ...
            if (!leadId)
                continue; // this email isn't with a known lead — skip
            try {
                ...
                dbLogger.warn(`Failed to log activity for gmail message ${msg.id}: ${err.message}`);
            }
        }
        connection.lastSyncedAt = new Date();
        await connection.save();
        dbLogger.info(`Gmail sync complete: user=${result.userId} email=${result.email} ` +
            `checked=${result.messagesChecked} logged=${result.activitiesLogged}`);
        return result;
    }
    catch (err) {
        result.error = err.message;
        dbLogger.error(`Gmail sync failed: user=${result.userId} email=${result.email} error=${result.error}`);
        return result;
    }
}
/* =====================================================
...
export async function syncAllGmailConnections() {
    ...
    dbLogger.info(`Gmail sync batch complete: total=${connections.length} succeeded=${succeeded} failed=${failed}`);
    return {
 ...
 */
export async function syncSingleUserGmail(userId) {
