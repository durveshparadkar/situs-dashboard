...
function getProvider(req) {
    ...
    if (!isValidProvider(provider)) {
        throw ApiError.badRequest("Invalid or unsupported integration provider");
    }
    return provider;
}
/**
 ...
 */
function optionalBodyString(body, field) {
    const value = body[field];
 ...
 */
function optionalMetadata(body) {
...
class IntegrationController {
    ...
    disconnect = asyncHandler(async (req, res) => {
        const organizationId = getOrganizationId(req);
        const provider = getProvider(req);
        const integration = await integrationService.disconnectIntegration(organizationId, provider);
        res.status(200).json({ success: true, data: integration });
    });
    /* ── GET INTEGRATIONS ── */
    findAll = asyncHandler(async (req, res) => {
        const organizationId = getOrganizationId(req);
        const integrations = await integrationService.getOrganizationIntegrations(organizationId);
