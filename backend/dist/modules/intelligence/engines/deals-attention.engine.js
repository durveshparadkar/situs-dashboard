...
// VERSIONING
// ============================================================
export const DEALS_ATTENTION_ENGINE_VERSION = "1.0.0";
// ============================================================
// CONFIG
// ============================================================
 ...
 */
export function detectDealAttention(signals) {
    ...
    // -----------------------------------------------------------
    if (signals.value >= ATTENTION_CONFIG.highValue.enterpriseValueMin &&
    ...
    // HIGH-VALUE RISK
    // -----------------------------------------------------------
    else if (signals.value >= ATTENTION_CONFIG.highValue.valueMin &&
    ...
    // -----------------------------------------------------------
    if (stage === ATTENTION_KNOWN_STAGES.NEGOTIATION &&
    ...
    // STALLED PROPOSAL
    // -----------------------------------------------------------
    if (stage === ATTENTION_KNOWN_STAGES.PROPOSAL_SENT &&
    ...
    // STALLED DISCOVERY — early stage going cold
    // -----------------------------------------------------------
    if (stage === ATTENTION_KNOWN_STAGES.DISCOVERY &&
    ...
    // -----------------------------------------------------------
    if (a >= ATTENTION_CONFIG.inactivity.days &&
        alerts.length === 0) {
        alerts.push(buildAlert({
            signals,
...
function normalizeStageName(stage) {
    ...
    if (upper === "PROPOSAL")
        return ATTENTION_KNOWN_STAGES.PROPOSAL_SENT;
    if (upper === "QUALIFYING")
        return ATTENTION_KNOWN_STAGES.QUALIFICATION;
    if (upper === "QUALIFY")
        return ATTENTION_KNOWN_STAGES.QUALIFICATION;
    if (upper === "DISCOVER")
        return ATTENTION_KNOWN_STAGES.DISCOVERY;
    return upper;
}
 ...
 */
export function detectAttentionAcrossDeals(deals) {
    const all = [];
 ...
 */
export function groupAttentionByDeal(alerts) {
 ...
 */
export function detectDealAttentionLegacy(deals) {
    const result = [];
    for (const d of deals) {
        ...
        for (const a of alerts) {
            ...
            let legacyType;
            if (a.type === ATTENTION_ALERT_TYPES.STALLED_NEGOTIATION ||
            ...
            else if (a.type === ATTENTION_ALERT_TYPES.HIGH_VALUE_RISK ||
...
//# sourceMappingURL=deals-attention.engine.js.map