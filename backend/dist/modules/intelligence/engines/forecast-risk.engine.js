...
// VERSIONING
// ============================================================
export const FORECAST_RISK_ENGINE_VERSION = "1.0.0";
// ============================================================
// CONFIG
// ============================================================
...
function normalizeStageName(stage) {
    ...
    if (upper === "PROPOSAL")
        return FORECAST_KNOWN_STAGES.PROPOSAL_SENT;
    if (upper === "CONTRACT")
        return FORECAST_KNOWN_STAGES.CONTRACT_SENT;
    return upper;
}
 ...
 */
function computeWeightedImpact(s) {
...
function detectSlipped(s, now) {
    ...
    return {
        ...
        revenueImpact: s.value,
        weightedImpact: computeWeightedImpact(s),
        message: s.name + " expected to close " + daysOverdue +
            " days ago — still open",
        recommendedAction: "Reset expected close date or escalate — slipped deals erode forecast credibility",
    };
}
function detectSlippingSoon(s, now) {
    if (!s.expectedCloseDate)
        return null;
    const daysToClose = daysBetween(s.expectedCloseDate, now);
    if (daysToClose <= 0)
        return null; // already slipped, handled by detectSlipped
    if (daysToClose > FORECAST_CONFIG.slippingSoon.daysToClose)
        return null;
    // Only flag if the deal isn't actively progressing
    if (s.lastActivityDays <= 2)
        return null;
    return {
        ...
        revenueImpact: s.value,
        weightedImpact: computeWeightedImpact(s),
        message: s.name + " expected to close in " + daysToClose +
...
function detectBestCaseErosion(s) {
    if (s.forecastCategory !== "best_case")
        return null;
    if (typeof s.probability !== "number")
        return null;
    if (s.probability > FORECAST_CONFIG.bestCaseErosion.probabilityMax)
        return null;
    if (s.lastActivityDays < FORECAST_CONFIG.bestCaseErosion.inactivityDays)
        return null;
    return {
        ...
        revenueImpact: s.value,
        weightedImpact: computeWeightedImpact(s),
        message: s.name + " in best_case at " + s.probability +
...
function detectMegaDealAtRisk(s, existingRisk, currency) {
    if (s.value < FORECAST_CONFIG.megaDeal.valueMin)
        return null;
    if (!existingRisk)
        return null; // only flag mega deals that already have other risk
    return {
...
export function detectForecastRisk(deals, currency = "INR") {
    ...
    if (riskByDeal.size >= 5 && totalAtRisk > 0) {
        ...
        if (topShare >= FORECAST_CONFIG.concentration.revenueShareMin) {
            factors.push({
                ...
                message: Math.round(topShare * 100) + "% of at-risk revenue concentrated in " +
                    topDealCount + " deal" + (topDealCount === 1 ? "" : "s"),
                recommendedAction: "Diversify focus — protect mega deals AND build mid-tier pipeline depth",
            });
        }
    }
    // -----------------------------------------------------------
    ...
    const uniqueDealRevenue = new Map();
    const uniqueDealWeighted = new Map();
    for (const f of factors) {
        if (f.type === FORECAST_RISK_TYPES.CONCENTRATION)
            continue;
        const key = String(f.dealId ?? f.dealName);
...
export function extractForecastSignals(input) {
    ...
    if (input.expectedCloseDate !== undefined)
        signals.expectedCloseDate = input.expectedCloseDate;
    if (input.forecastCategory !== undefined)
        signals.forecastCategory = input.forecastCategory;
    if (input.status !== undefined)
        signals.status = input.status;
    if (input.weightedValue !== undefined)
        signals.weightedValue = input.weightedValue;
    return signals;
}
// ============================================================
 ...
 */
export function detectForecastRiskLegacy(deals) {
    ...
    if (result.summary.totalRevenueAtRisk === 0)
        return null;
    return {
...
//# sourceMappingURL=forecast-risk.engine.js.map