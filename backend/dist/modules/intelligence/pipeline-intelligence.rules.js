...
function makeRecId(dealId, ruleCode) {
...
===================================================== */
export const staleDealRule = ({ deal }) => {
    if (deal.status !== "open")
        return [];
...
===================================================== */
export const closeDateSlippageRule = ({ deal }) => {
...
===================================================== */
export const stageStagnationRule = ({ deal, orgAvgVelocity }) => {
    if (deal.status !== "open")
        return [];
...
===================================================== */
export const highValueAtRiskRule = ({ deal, orgAvgDealSize }) => {
    if (deal.status !== "open")
        return [];
    const benchmark = orgAvgDealSize ?? 500_000;
    const isHighValue = deal.value >= benchmark * 2;
    const isAtRisk = ["high", "critical"].includes(deal.riskLevel);
    if (!isHighValue || !isAtRisk)
        return [];
...
===================================================== */
export const lowProbabilityLateStageRule = ({ deal }) => {
    if (deal.status !== "open")
        return [];
    ...
    if (ageDays < 30 || deal.probability >= 30)
        return [];
    return [
        {
...
===================================================== */
export const competitiveThreatRule = ({ deal }) => {
    if (deal.status !== "open")
        return [];
    const strongCompetitors = (deal.competitors ?? []).filter(c => c.strength === "strong");
    if (!strongCompetitors.length)
        return [];
    return [
        {
...
===================================================== */
export const missingNextStepRule = ({ deal }) => {
    if (deal.status !== "open")
        return [];
...
//# sourceMappingURL=pipeline-intelligence.rules.js.map