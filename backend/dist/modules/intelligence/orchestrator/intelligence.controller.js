...
function requireObjectIdParam(req, name) {
    const value = req.params[name];
    if (typeof value !== "string" || !mongoose.Types.ObjectId.isValid(value)) {
        throw new AppError("Invalid or missing " + name, HttpStatus.BAD_REQUEST, "INVALID_ID");
    }
    return value;
}
/* =====================================================
...
class IntelligenceController {
    ...
    refresh = async (req, res, next) => {
        try {
            ...
            dbLogger.info("Intelligence refresh: org=" + actor.organizationId +
    ...
    refreshDeal = async (req, res, next) => {
        try {
            const actor = requireActor(req);
            const dealId = requireObjectIdParam(req, "dealId");
            const dealRisk = await intelligenceService.refreshOneDeal(dealId, actor.organizationId, actor.userId);
            if (!dealRisk) {
                throw new AppError("Deal not found in intelligence scope", HttpStatus.NOT_FOUND, "DEAL_NOT_FOUND");
            }
