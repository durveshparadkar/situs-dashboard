...
const ORG_CONFIG = {
    ...
    privilegedRoles: ["SUPER_ADMIN", "ORG_ADMIN"],
    superAdminOnly: ["SUPER_ADMIN"],
};
/* =====================================================
   ALLOWLISTS
===================================================== */
const VALID_PLANS = ["SMALL_BUSINESS", "PRO", "ENTERPRISE"];
...
function requireAuth(req) {
    ...
    if (!userId) {
        throw new AppError("User missing identity", HttpStatus.UNAUTHORIZED, "UNAUTHORIZED");
    }
    return {
        userId,
        organizationId,
 ...
 */
function requireOrg(actor) {
    if (!actor.organizationId) {
        throw new AppError("User is not attached to an organization", HttpStatus.BAD_REQUEST, "NO_ORGANIZATION");
    }
}
function requireRole(actor, allowed) {
    if (!allowed.includes(actor.role)) {
        throw new AppError("Forbidden — insufficient role", HttpStatus.FORBIDDEN, "FORBIDDEN");
    }
}
...
class OrganizationController {
    ...
    create = asyncHandler(async (req, res, _next) => {
        ...
        if (actor.organizationId) {
            throw new AppError("User is already attached to an organization", HttpStatus.CONFLICT, "ALREADY_IN_ORG");
        }
        ...
        try {
            await session.withTransaction(async () => {
                /* ── 1. Create the org ── */
                const orgSvc = OrganizationService;
                ...
                const orgIdStr = typeof org._id === "string" ? org._id : org._id.toString();
                ...
                const billingSvc = OrganizationService;
                await billingSvc.updateBilling(orgIdStr, isEnterprise ? "ACTIVE" : "TRIAL", {
                    _id: actor.userId,
                    role: "SUPER_ADMIN",
            ...
            if (!createdOrg) {
                throw new AppError("Organization creation failed", HttpStatus.INTERNAL, "TRANSACTION_FAILED");
            }
        }
        finally {
            await session.endSession();
        }
        const finalOrg = createdOrg;
        const finalOrgId = typeof finalOrg._id === "string" ? finalOrg._id : finalOrg._id.toString();
        ...
        try {
            ...
            dbLogger.warn(`Audit log failed (non-fatal): org=${finalOrgId} ` +
                `error=\`${err?.message ?? "unknown"}\``);
        }
        dbLogger.info(`Organization created: id=${finalOrgId} name=${name} ` +
    ...
    ===================================================== */
    getCurrent = asyncHandler(async (req, res, _next) => {
        const actor = requireAuth(req);
        requireOrg(actor);
    ...
    update = asyncHandler(async (req, res, _next) => {
        ...
        const updated = await OrganizationService.update(validated, buildServiceUser(actor));
        if (!updated) {
            throw new AppError("Organization not found", HttpStatus.NOT_FOUND, "ORG_NOT_FOUND");
        }
        /* Audit log */
        try {
            ...
            dbLogger.warn(`Audit log failed (non-fatal): org=${actor.organizationId} ` +
                `error=\`${err?.message ?? "unknown"}\``);
        }
        dbLogger.info(`Organization updated: id=${actor.organizationId} ` +
    ...
    delete = asyncHandler(async (req, res, _next) => {
        ...
        if (confirmation !== "DELETE_ORGANIZATION") {
            throw new AppError("Confirmation required. Send { confirmation: 'DELETE_ORGANIZATION' } in body.", HttpStatus.BAD_REQUEST, "CONFIRMATION_REQUIRED");
        }
        ...
        dbLogger.warn(`Organization deleted: id=${actor.organizationId} actor=${actor.userId}`);
        res.status(HttpStatus.OK).json({
            success: true,
    ...
    ===================================================== */
    listAll = asyncHandler(async (req, res, _next) => {
        const actor = requireAuth(req);
        requireRole(actor, ORG_CONFIG.superAdminOnly);
        const page = Math.max(parseInt(String(req.query.page ?? "1"), 10) || 1, 1);
        const limit = Math.min(Math.max(parseInt(String(req.query.limit ?? String(ORG_CONFIG.pagination.defaultLimit)), 10) || ORG_CONFIG.pagination.defaultLimit, 1), ORG_CONFIG.pagination.maxLimit);
        /* Support optional service signature: (user, { page, limit }) — falls back
           to (user) if service doesn't accept pagination yet. */
        const svc = OrganizationService;
        const data = await svc.listAll(buildServiceUser(actor), { page, limit });
        dbLogger.info(`Organization list (admin): actor=${actor.userId} ` +
    ...
    ===================================================== */
    transferOwnership = asyncHandler(async (req, res, _next) => {
        const actor = requireAuth(req);
        requireOrg(actor);
        requireRole(actor, ORG_CONFIG.privilegedRoles);
        ...
        if (!newOwnerId || !mongoose.Types.ObjectId.isValid(newOwnerId)) {
            throw new AppError("Valid newOwnerId is required", HttpStatus.BAD_REQUEST, "INVALID_USER_ID");
        }
        if (newOwnerId === actor.userId) {
            throw new AppError("Cannot transfer ownership to yourself", HttpStatus.BAD_REQUEST, "INVALID_TRANSFER");
        }
        ...
        if (!newOwner) {
            throw new AppError("New owner must be a member of your organization", HttpStatus.BAD_REQUEST, "NOT_ORG_MEMBER");
        }
        const svc = OrganizationService;
        if (typeof svc.transferOwnership !== "function") {
            throw new AppError("Ownership transfer not available", HttpStatus.NOT_FOUND, "NOT_AVAILABLE");
        }
        ...
        dbLogger.warn(`Org ownership transferred: org=${actor.organizationId} ` +
