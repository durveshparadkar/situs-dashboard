...
async function findOrCreateDefaultPipeline(orgId, session) {
...
class DealService {
        ...
        try {
            await session.withTransaction(async () => {
                ...
                const stage = data.stageId
                    ? pipeline.stages.find((s) => s._id.toString() === data.stageId)
            ...
            const createdDealId = createdDeal._id.toString();
            void DealRiskService.calculateRisk(createdDealId).catch((riskErr) => {
                logger.error({ dealId: createdDealId, error: riskErr.message }, "Risk calculation failed post-create");
            });
            return createdDeal;
        }
        finally {
    ...
    async getDeals(orgId, filters = {}) {
        ...
        if (filters.stage)
            query.stageId = toObjectId(filters.stage);
        if (filters.pipelineId)
            query.pipelineId = toObjectId(filters.pipelineId);
        if (filters.ownerId)
            query.assignedTo = toObjectId(filters.ownerId);
        if (filters.status)
    ...
    /* ── GET DEAL BY ID ── */
    async getDealById(dealId, orgId) {
    ...
    /* ── UPDATE DEAL ── */
    async updateDeal(dealId, updates, userId, orgId) {
        if (!isValidObjectId(dealId)) {
            throw new ServiceError("Invalid deal ID", 400, "INVALID_ID");
        }
        const query = { _id: dealId, isDeleted: false };
        if (orgId)
        ...
        const isCollaborator = deal.collaborators?.some((c) => c.toString() === userId);
        ...
        if (updates.pipelineId || updates.stageId) {
            ...
            const stage = pipeline.stages.find((s) => s._id.toString() === stageIdToUse);
            if (!stage) {
                throw new ServiceError("Invalid stage for this pipeline", 400, "INVALID_STAGE");
            }
        ...
        void DealRiskService.calculateRisk(dealId).catch((riskErr) => {
            logger.error({ dealId, error: riskErr.message }, "Risk calculation failed post-update");
        });
        return deal;
    }
    /* ── UPDATE DEAL STAGE ── */
    async updateDealStage(dealId, stageId, userId, orgId) {
    ...
    /* ── SOFT DELETE ── */
    async softDeleteDeal(dealId, userId, orgId) {
        if (!isValidObjectId(dealId)) {
            throw new ServiceError("Invalid deal ID", 400, "INVALID_ID");
        }
    ...
    /* ── RESTORE ── */
    async restoreDeal(dealId, userId, orgId) {
        const deal = await Deal.findOne({
            _id: dealId,
            organizationId: orgId,
            isDeleted: true,
    ...
    /* ── STATS ── */
    async getDealStats(orgId) {
        const matchOpen = {
            organizationId: new mongoose.Types.ObjectId(orgId),
            isDeleted: false,
        };
        const [overview, byStage, byStatus, byRiskLevel, winRateData] = await Promise.all([
            Deal.aggregate([
                { $match: matchOpen },
                {
                    $group: {
        ...
        const winRate = winRateData[0]?.total > 0
    ...
    /* ── BULK UPDATE STAGE ── */
    async bulkUpdateStage(dealIds, stageId, userId, orgId) {
        if (!dealIds.length)
        ...
        logger.info({ orgId, userId, count: result.modifiedCount, stageId }, "Bulk stage update");
