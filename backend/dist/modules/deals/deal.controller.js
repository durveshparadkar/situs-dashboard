...
import { validateCreateDeal, validateUpdateDeal, } from "./deal.validation.js";
 ...
 */
function getDealId(req) {
    const id = req.params.id;
    if (!id || !id.trim()) {
        throw new AppError("Deal ID required", HttpStatus.BAD_REQUEST, "MISSING_ID");
    }
    if (!mongoose.Types.ObjectId.isValid(id)) {
        throw new AppError("Invalid Deal ID format", HttpStatus.BAD_REQUEST, "INVALID_ID");
    }
    return id;
}
...
class DealController {
    ...
    async createDeal(req, res, next) {
        try {
            ...
            const deal = await DealService.createDeal(req.body, userId, organizationId);
            const dealId = deal?._id !== undefined
                ? String(deal._id)
                : "unknown";
            log.info({ dealId, userId, organizationId }, "Deal created successfully");
    ...
    ===================================================== */
    async importDeals(req, res, next) {
        try {
            const { userId, organizationId } = getUser(req);
            const rows = req.body?.rows;
            if (!Array.isArray(rows) || rows.length === 0) {
                throw new AppError("Request body must contain a non-empty 'rows' array", HttpStatus.BAD_REQUEST, "INVALID_PAYLOAD");
            }
            if (rows.length > DEAL_CONFIG.import.maxRows) {
                throw new AppError("Cannot import more than " +
                    DEAL_CONFIG.import.maxRows +
                    " deals at once", HttpStatus.BAD_REQUEST, "TOO_MANY_ROWS");
            }
            log.info({ userId, organizationId, rowCount: rows.length }, "Deal import initiated");
            let importedCount = 0;
            const skipped = [];
            for (let i = 0; i < rows.length; i++) {
                ...
                if (probability !== undefined)
                    dealInput.probability = probability;
                try {
                    await DealService.createDeal(dealInput, userId, organizationId);
                    importedCount++;
                }
                catch (err) {
                    skipped.push({
    ...
    async getDeals(req, res, next) {
        try {
            ...
            const limit = getNumber(req.query.limit, DEAL_CONFIG.pagination.defaultLimit, { min: 1, max: DEAL_CONFIG.pagination.maxLimit });
            /* Filters */
            ...
            const minValue = req.query.minValue !== undefined
                ? getNumber(req.query.minValue, 0, { min: 0 })
                : undefined;
            const maxValue = req.query.maxValue !== undefined
            ...
            const sortBy = getEnumParam(req.query.sortBy, VALID_SORT_FIELDS, "createdAt") ??
                "createdAt";
            const sortOrder = getEnumParam(req.query.sortOrder, VALID_SORT_ORDERS, "desc") ?? "desc";
            log.info({ userId, organizationId, page, limit, sortBy }, "Fetching deals");
    ...
    async getDealById(req, res, next) {
        try {
            ...
            if (!deal) {
                throw new AppError("Deal not found", HttpStatus.NOT_FOUND, "DEAL_NOT_FOUND");
            }
            res.status(HttpStatus.OK).json({
                success: true,
                data: deal,
    ...
    async updateDeal(req, res, next) {
        try {
            ...
            log.info({ dealId, userId, fields: Object.keys(body) }, "Deal update initiated");
            validateUpdateDeal(body);
            const deal = await DealService.updateDeal(dealId, body, userId, organizationId);
            if (!deal) {
                throw new AppError("Deal not found", HttpStatus.NOT_FOUND, "DEAL_NOT_FOUND");
            }
    ...
    ===================================================== */
    async restoreDeal(req, res, next) {
        try {
            const dealId = getDealId(req);
            const { userId, organizationId } = getUser(req);
            const svc = DealService;
            ...
            if (!deal) {
                throw new AppError("Deal not found or not deleted", HttpStatus.NOT_FOUND, "DEAL_NOT_FOUND");
            }
    ...
    async updateDealStage(req, res, next) {
        try {
            ...
            if (!stage) {
                throw new AppError("Stage is required", HttpStatus.BAD_REQUEST, "MISSING_STAGE");
            }
            ...
            if (typeof svc.updateStage === "function") {
                deal = await svc.updateStage(dealId, stage, userId, organizationId, reason);
            }
            else {
                deal = await DealService.updateDeal(dealId, { stage }, userId, organizationId);
            }
            if (!deal) {
                throw new AppError("Deal not found", HttpStatus.NOT_FOUND, "DEAL_NOT_FOUND");
            }
            log.info({ dealId, stage, userId, reason: reason ?? "none" }, "Deal stage updated");
            res.status(HttpStatus.OK).json({
                success: true,
                data: deal,
    ...
    ===================================================== */
    async getDealStats(req, res, next) {
        try {
            const { userId, organizationId } = getUser(req);
            const svc = DealService;
            if (typeof svc.getDealStats !== "function") {
                throw new AppError("Stats endpoint not available", HttpStatus.NOT_FOUND, "NOT_AVAILABLE");
            }
    ...
    async bulkUpdateStage(req, res, next) {
        try {
            ...
            if (!Array.isArray(updates) || updates.length === 0) {
                throw new AppError("Request body must contain a non-empty 'updates' array", HttpStatus.BAD_REQUEST, "INVALID_PAYLOAD");
            }
            if (updates.length > 200) {
                throw new AppError("Cannot update more than 200 deals at once", HttpStatus.BAD_REQUEST, "TOO_MANY_UPDATES");
            }
            ...
            const validated = updates.map((u, idx) => {
                if (!u ||
                    ...
                    !u.stage.trim()) {
                    throw new AppError(`Invalid update at index ${idx}`, HttpStatus.BAD_REQUEST, "INVALID_UPDATE_ENTRY");
                }
            ...
            if (typeof svc.bulkUpdateStage === "function") {
                ...
                const settled = await Promise.allSettled(validated.map((v) => DealService.updateDeal(v.dealId, { stage: v.stage }, userId, organizationId)));
                result = {
                    updated: settled.filter((r) => r.status === "fulfilled").length,
                    failed: settled.filter((r) => r.status === "rejected").length,
                };
            }
            log.info({ userId, organizationId, total: validated.length, ...result }, "Bulk stage update complete");
            res.status(HttpStatus.OK).json({
                success: true,
                data: result,
