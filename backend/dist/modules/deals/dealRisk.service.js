...
class DealRiskService {
    ...
    async calculateRisk(dealId) {
        ...
        const riskFactors = signals.map((s) => ({
            factor: s.factor,
            weight: s.weight,
            detectedAt: new Date(),
            resolved: false,
        }));
        const changed = previousScore !== riskScore || previousLevel !== riskLevel;
        if (!changed) {
            return {
                dealId,
                riskScore,
        ...
        await deal.save();
        logger.info({ dealId, riskScore, riskLevel, previousScore, previousLevel, signalCount: signals.length }, "Risk recalculated");
    ...
    detectHighValueSensitivity(deal) {
    ...
    detectStageStagnation(deal) {
        const lastTransition = deal.stageHistory?.[deal.stageHistory.length - 1]?.enteredAt ??
            deal.updatedAt;
    ...
    detectCloseDateRisk(deal) {
        ...
        if (daysOut < 0) {
            ...
            const isShaky = (deal.probability ?? 0) < 60 ||
    ...
    /* ── ALERTING ── */
    async handleAlerts(deal, previousLevel, signals) {
        const levelOrder = {
        ...
        try {
            ...
            logger.error({ dealId: deal._id, riskLevel: deal.riskLevel, error: err.message }, "Alert creation failed in risk engine");
        }
    }
    /* ── BATCH RECALC ── */
    async recalculateForOrg(orgId) {
        ...
        for await (const deal of cursor) {
            ...
            try {
                ...
                errors++;
                logger.error({ dealId: deal._id, error: err.message }, "Batch risk calc error");
            }
        }
...
//# sourceMappingURL=dealRisk.service.js.map