...
const requireAnalyticsAccess = (req, res, next) => {
    ...
    if (reqWithPlan.planContext) {
        ...
        const userPlan = String(u?.plan ?? "FREE").toUpperCase();
        hasAccess = hasFeature(userPlan, "advancedAnalytics");
        planTier = userPlan;
    }
    if (!hasAccess) {
        dbLogger.info("Analytics access denied — plan gating: user=" +
 ...
 */
router.get("/", ANALYTICS_LIMIT, authorizeMw("READ_REPORTS"), cache((req) => "analytics:dashboard:" + (req.query.trendMonths ?? "12"), { ttl: 60 }), analyticsController.getDashboard);
/**
 * @route   GET /api/analytics/summary
 * @desc    Summary metrics only (lightweight)
 */
router.get("/summary", ANALYTICS_LIMIT, authorizeMw("READ_REPORTS"), cache((_req) => "analytics:summary", { ttl: 60 }), analyticsController.getSummary);
/**
 * @route   GET /api/analytics/funnel
 * @desc    Funnel stages only
 */
router.get("/funnel", ANALYTICS_LIMIT, authorizeMw("READ_REPORTS"), cache((_req) => "analytics:funnel", { ttl: 60 }), analyticsController.getFunnel);
/**
 * @route   GET /api/analytics/trend
 * @desc    Revenue trend chart data
 */
router.get("/trend", ANALYTICS_LIMIT, authorizeMw("READ_REPORTS"), cache((req) => "analytics:trend:" + (req.query.trendMonths ?? "12"), { ttl: 60 }), analyticsController.getTrend);
/**
 * @route   GET /api/analytics/insights
 * @desc    Insights + top actions
 */
router.get("/insights", ANALYTICS_LIMIT, authorizeMw("READ_REPORTS"), cache((_req) => "analytics:insights", { ttl: 60 }), analyticsController.getInsights);
