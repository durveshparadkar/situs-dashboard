 ...
 */
function percentage(numerator, denominator, decimals = 1) {
    if (denominator <= 0)
        return 0;
...
class AnalyticsService {
     ...
     */
    async getSummary(organizationId, window) {
     ...
     */
    async getFunnel(organizationId) {
        const orgId = toObjectId(organizationId);
     ...
     */
    async getRevenueTrend(organizationId, months) {
        const orgId = toObjectId(organizationId);
        ...
        for (let i = 0; i < trendMonths; i++) {
            ...
            const monthData = monthly.find((r) => r._id.year === year && r._id.month === monthNum);
            series.push({
     ...
     */
    async getInsights(organizationId, window) {
        const orgId = toObjectId(organizationId);
        const insights = [];
        const DealModel = Deal;
        // Stale deals (no activity > 14 days, status open)
        const staleThreshold = new Date(Date.now() - ANALYTICS_CONFIG.staleDaysThreshold * 24 * 60 * 60 * 1000);
        const [stalledDealsAgg, atRiskValueAgg, thisMonthRevenueAgg, lastMonthRevenueAgg, avgDealAgeAgg,] = await Promise.all([
     ...
     */
    async getTopActions(organizationId) {
        const orgId = toObjectId(organizationId);
        const actions = [];
        const DealModel = Deal;
     ...
     * in parallel.
     */
    async getDashboard(organizationId, options = {}) {
        const now = new Date();
        const window = options.window ?? {
            fromDate: addMonths(now, -ANALYTICS_CONFIG.defaultTrendMonths),
            toDate: now,
        };
        const trendMonths = clampMonths(options.trendMonths, ANALYTICS_CONFIG.defaultTrendMonths);
        try {
            const [summary, funnelStages, revenueTrend, insights, topActions] = await Promise.all([
            ...
            dbLogger.error("Analytics dashboard aggregation failed: org=" + organizationId +
