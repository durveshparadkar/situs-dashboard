...
function validatePassword(password) {
    ...
    if (!password || password.length === 0) {
        ...
        dbLogger.warn("SUPER_ADMIN_PASSWORD not set — using fallback (dev only)");
        return;
    }
    if (password.length < SEED_CONFIG.password.minLength) {
        throw new Error(`SUPER_ADMIN_PASSWORD must be at least ${SEED_CONFIG.password.minLength} characters`);
    }
    const hasLetter = /[A-Za-z]/.test(password);
    const hasNumber = /[0-9]/.test(password);
    const hasSymbol = /[!@#$%^&*()_+\-=[\]{};':"\\|,.<>/?]/.test(password);
    if (!hasLetter || !hasNumber || (isProd && !hasSymbol)) {
        throw new Error("SUPER_ADMIN_PASSWORD must contain letters, numbers" +
...
async function ensurePlatformOrganization() {
    ...
    dbLogger.info("Platform organization created: id=" + String(created._id) +
        " name=" + SEED_CONFIG.organization.name);
    return {
...
async function ensureSuperAdminRole() {
    ...
    dbLogger.info("SUPER_ADMIN role created: id=" + String(created._id));
    return {
        id: String(created._id),
 ...
 */
async function ensureSuperAdminUser(organizationId, roleId) {
    ...
    const rawPassword = SEED_CONFIG.superAdmin.password ||
        (process.env.NODE_ENV !== "production" ? "ChangeMe@123" : "");
    if (!rawPassword) {
        throw new Error("Cannot seed super admin: no password configured in production");
    }
    const hashedPassword = await bcrypt.hash(rawPassword, SEED_CONFIG.password.bcryptRounds);
    ...
    dbLogger.info("Super admin user created: " + JSON.stringify(safeLogUser(created)));
    return {
        id: String(created._id),
...
export async function seedSuperAdmin() {
    ...
    try {
        ...
        const result = { organization, role, user };
        // Summary log
        const summary = "org=" + (organization.created ? "created" : "exists") +
        ...
        dbLogger.error("Super admin seed failed: " +
...
async function runCli() {
    ...
    try {
        ...
        try {
            ...
            dbLogger.warn("Error during disconnect: " + (err?.message ?? "unknown"));
        }
