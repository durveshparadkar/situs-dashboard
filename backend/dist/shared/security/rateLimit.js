...
function shouldSkip(req) {
    ...
    if (GLOBAL_LIMITER_CONFIG.skipPaths.includes(path)) {
        return true;
    }
    // Test mode
    if (GLOBAL_LIMITER_CONFIG.skipInTest)
        return true;
    // Dev mode (unless explicitly enabled)
    if (isDev && !GLOBAL_LIMITER_CONFIG.enableInDev)
        return true;
    // Bypass roles — platform staff with legitimate heavy use
    const role = getNormalizedRole(req);
    if (GLOBAL_LIMITER_CONFIG.bypassRoles.includes(role)) {
        return true;
    }
    return false;
}
...
const memoryLimiter = rateLimit({
    ...
    handler: (req, res) => {
        ...
        dbLogger.warn("Global rate limit hit: " +
            "key=" + key +
            " method=" + req.method +
 ...
 * resources.
 */
export const globalLimiter = memoryLimiter;
