...
export const LEAD_STAGE_TO_GROUP = {
...
export const ACTIVE_LEAD_STAGES = LEAD_STAGE_ORDER;
export const HOLDING_LEAD_STAGES = [
    LeadStage.ON_HOLD,
    LeadStage.NURTURE,
    LeadStage.COLD,
];
...
export const LEAD_SOURCE_TO_CATEGORY = {
    // Inbound digital
...
export const LEAD_PRIORITY_ORDER = [
    LeadPriority.LOW,
    LeadPriority.MEDIUM,
    LeadPriority.HIGH,
    LeadPriority.URGENT,
...
export const LEAD_LOST_REASON_TO_CATEGORY = {
...
// ============================================================
export function isValidLeadStage(value) {
    if (typeof value !== "string")
        return false;
    return Object.values(LeadStage).includes(value);
}
...
export function isValidLeadPriority(value) {
    if (typeof value !== "string")
        return false;
    return Object.values(LeadPriority).includes(value);
}
export function isValidLeadTemperature(value) {
    if (typeof value !== "string")
        return false;
    return Object.values(LeadTemperature).includes(value);
}
export function isValidLeadLostReason(value) {
    if (typeof value !== "string")
        return false;
    return Object.values(LeadLostReason).includes(value);
}
export function isValidCrmSystem(value) {
    if (typeof value !== "string")
        return false;
    return Object.values(CrmSystem).includes(value);
}
/**
...
export function normalizeLeadStage(value) {
    ...
    if (Object.values(LeadStage).includes(cleaned)) {
        return cleaned;
    }
    const aliases = {
        // Common synonyms
...
export function normalizeLeadSource(value) {
    if (typeof value !== "string")
        return null;
    const cleaned = value.trim().toUpperCase().replace(/[-\s.]+/g, "_");
    if (Object.values(LeadSource).includes(cleaned)) {
        return cleaned;
    }
    const aliases = {
        // Paid digital
 ...
 */
export function canTransitionLeadStage(from, to) {
    if (from === to)
        return { allowed: true };
    if (TERMINAL_LEAD_STAGES.includes(from)) {
        return {
            allowed: false,
 ...
 */
export function getLeadProgress(stage) {
    if (stage === LeadStage.WON)
        return 1;
    if (stage === LeadStage.LOST || stage === LeadStage.DISQUALIFIED)
        return 0;
    if (HOLDING_LEAD_STAGES.includes(stage))
        return 0;
 ...
 */
export function getNextLeadStage(current) {
    if (TERMINAL_LEAD_STAGES.includes(current))
        return null;
    if (HOLDING_LEAD_STAGES.includes(current))
        return null;
    const idx = LEAD_STAGE_ORDER.indexOf(current);
    if (idx === -1 || idx >= LEAD_STAGE_ORDER.length - 1)
        return null;
 ...
 */
export function getDefaultWinProbability(stage) {
    return LEAD_STAGE_META[stage]?.defaultWinProbability ?? 0;
}
export function getLeadSourceCategory(source) {
    return LEAD_SOURCE_TO_CATEGORY[source] ?? LEAD_SOURCE_CATEGORY.OTHER;
}
export function getLeadLostReasonCategory(reason) {
...
//# sourceMappingURL=lead.enums.js.map