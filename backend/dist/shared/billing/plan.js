 ...
 */
export const PLAN_TIERS = ["FREE", "SMALL", "PRO", "ENTERPRISE"];
/**
 * Plan tiers that require billing (i.e. non-FREE).
 */
export const PAID_PLAN_TIERS = ["SMALL", "PRO", "ENTERPRISE"];
// ============================================================
// PLAN CATALOG
// ============================================================
/**
...
export function getPlan(tier) {
    if (!tier)
        return null;
    const normalized = (tier.toUpperCase() === "SMALL_BUSINESS" ? "SMALL" : tier.toUpperCase());
 ...
 */
export function getStripePriceId(tier, cycle) {
    const plan = PLANS[tier];
    if (!plan || !plan.purchasable)
        return null;
    const envKey = cycle === "monthly"
 ...
 */
export function getQuota(tier, quota) {
    const plan = getPlan(typeof tier === "string" ? tier : null);
    if (!plan)
        return 0;
...
export function isPaidPlan(tier) {
    if (!tier)
        return false;
    return PAID_PLAN_TIERS.includes(tier.toUpperCase());
}
/**
 ...
 */
export function formatPrice(amountInSmallestUnit, currency = DEFAULT_CURRENCY) {
