...
function requireRole(actor, allowed) {
    if (!allowed.includes(actor.role)) {
        throw new AppError("Insufficient role for this action", HttpStatus.FORBIDDEN, "FORBIDDEN");
    }
}
...
function buildFilter(actor, q) {
    ...
    if (q.fromDate || q.toDate) {
        ...
        if (q.toDate)
            dateFilter.$lte = new Date(q.toDate);
...
class AuditController {
    ...
    // -----------------------------------------------------------
    getAuditLogs = asyncHandler(async (req, res, _next) => {
        ...
        const limit = getNumber(validated.limit, AUDIT_CONFIG.pagination.defaultLimit, { min: 1, max: AUDIT_CONFIG.pagination.maxLimit });
    ...
    // -----------------------------------------------------------
    getAuditLogById = asyncHandler(async (req, res, _next) => {
        const actor = requireAuth(req);
        requireRole(actor, AUDIT_CONFIG.viewerRoles);
    ...
    // -----------------------------------------------------------
    getAuditStats = asyncHandler(async (req, res, _next) => {
        const actor = requireAuth(req);
        requireRole(actor, AUDIT_CONFIG.viewerRoles);
        const validated = runSchema(listQuerySchema, req.query);
        const filter = buildFilter(actor, validated);
    ...
    // -----------------------------------------------------------
    exportAuditLogs = asyncHandler(async (req, res, _next) => {
        const actor = requireAuth(req);
        ...
        dbLogger.warn("Audit export: actor=" + actor.userId +
            " org=" + actor.organizationId +
            " format=" + format +
            " rows=" + logs.length);
        const filename = "audit-export-" + actor.organizationId +
    ...
    // -----------------------------------------------------------
    getUserAuditLogs = asyncHandler(async (req, res, _next) => {
        const actor = requireAuth(req);
        requireRole(actor, AUDIT_CONFIG.viewerRoles);
        ...
        const limit = getNumber(req.query.limit, AUDIT_CONFIG.pagination.defaultLimit, { min: 1, max: AUDIT_CONFIG.pagination.maxLimit });
        const filter = {
            organizationId: actor.organizationId,
            userId,
        };
    ...
    // -----------------------------------------------------------
    getResourceAuditLogs = asyncHandler(async (req, res, _next) => {
        const actor = requireAuth(req);
        requireRole(actor, AUDIT_CONFIG.viewerRoles);
