...
const requireAdmin = (req, res, next) => {
    ...
    if (!ADMIN_ROLES.includes(role)) {
        dbLogger.warn(`Audit access denied: user=${req.user?.id ?? "anon"} role=${role}`);
        res.status(403).json({
            success: false,
            error: {
                code: "FORBIDDEN",
 ...
 */
router.get("/stats", STATS_LIMIT, authorizeMw(AUDIT_READ_PERM), auditController.getAuditStats);
/**
 ...
 */
router.get("/export", EXPORT_LIMIT, authorizeMw(AUDIT_READ_PERM), auditController.exportAuditLogs);
/**
 * @route   GET /audit-logs/user/:userId
 * @desc    Audit log history for a specific user
 * @access  Admin+ + READ_AUDIT
 */
router.get("/user/:userId", RESOURCE_LIMIT, authorizeMw(AUDIT_READ_PERM), validateObjectId("userId"), auditController.getUserAuditLogs);
/**
 * @route   GET /audit-logs/resource/:resource/:resourceId
 * @desc    Audit log history for a specific entity (e.g. /resource/DEAL/abc123)
 * @access  Admin+ + READ_AUDIT
 */
router.get("/resource/:resource/:resourceId", RESOURCE_LIMIT, authorizeMw(AUDIT_READ_PERM), validateObjectId("resourceId"), auditController.getResourceAuditLogs);
/* =====================================================
 ...
 */
router.get("/", READ_LIMIT, authorizeMw(AUDIT_READ_PERM), auditController.getAuditLogs);
/**
 * @route   GET /audit-logs/:id
 * @desc    Single audit log entry by ID
 * @access  Admin+ + READ_AUDIT
 */
router.get("/:id", READ_LIMIT, authorizeMw(AUDIT_READ_PERM), validateObjectId("id"), auditController.getAuditLogById);
