 ...
 */
function buildCacheKey(req, baseKey, varyByUser, varyByOrg) {
    const parts = [CACHE_CONFIG.keyPrefix, baseKey];
    if (varyByOrg) {
 ...
 */
function serializeForCache(body, statusCode) {
    try {
        const payload = {
            statusCode,
            body,
 ...
 */
function deserializeFromCache(raw) {
    try {
 ...
 *   );
 */
export function cache(keyBuilder, options = {}) {
    const ttl = Math.min(Math.max(options.ttl ?? CACHE_CONFIG.defaultTtlSeconds, 1), CACHE_CONFIG.maxTtlSeconds);
    ...
    return async (req, res, next) => {
        ...
        // -------------------------------------------------------
        if (!CACHE_CONFIG.cacheableMethods.includes(req.method)) {
        ...
        if (skipIf) {
            try {
                ...
                dbLogger.warn("Cache skipIf predicate threw: " +
                    (err?.message ?? "unknown"));
        ...
        try {
            ...
            if (typeof baseKey !== "string" || baseKey.length === 0) {
                ...
                dbLogger.warn("Cache keyBuilder returned invalid key for " + req.method + " " + req.path);
                next();
                return;
            }
            ...
            dbLogger.warn("Cache keyBuilder threw: " +
                (err?.message ?? "unknown"));
            next();
            return;
        }
        ...
        // -------------------------------------------------------
        const cached = await withTimeout(redis.get(cacheKey), CACHE_CONFIG.redisTimeoutMs);
        if (cached) {
        ...
        res.json = function (body) {
            ...
            const isCacheableStatus = CACHE_CONFIG.cacheableStatusCodes.includes(statusCode);
            ...
            if (shouldCache) {
                try {
                    ...
                    dbLogger.warn("Cache shouldCache predicate threw: " +
                        (err?.message ?? "unknown"));
                    return result;
                }
            }
            ...
            redis.setex(cacheKey, ttl, serialized.serialized).catch((err) => {
                dbLogger.warn("Cache write failed: key=" + cacheKey.slice(0, 100) +
...
export async function invalidateCacheKey(key) {
    ...
    try {
        ...
        dbLogger.warn("Cache invalidation failed: key=" + fullKey +
            " err=" + (err?.message ?? "unknown"));
    }
}
/**
...
export async function invalidateCachePattern(pattern) {
    ...
    try {
        ...
        dbLogger.warn("Cache pattern invalidation failed: pattern=" + fullPattern +
            " err=" + (err?.message ?? "unknown"));
        return deletedCount;
    }
}
 ...
 */
export async function invalidateOrgCache(organizationId) {
