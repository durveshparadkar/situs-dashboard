...
export function authorize(first, ...rest) {
    ...
    const allowed = Array.from(new Set(allRoles
    ...
    return (req, res, next) => {
        ...
        if (ROLE_GUARD_CONFIG.bypassRoles.includes(role)) {
            next();
            return;
        }
        // Role mismatch
        if (!allowed.includes(role)) {
            dbLogger.warn("Role-gate deny: user=" + userId +
                " role=" + role +
                " required=[" + allowed.join(",") + "]");
            sendAuthError(res, 403, "FORBIDDEN", "Access denied. Insufficient permissions.", { required: allowed });
            return;
        }
        next();
    };
