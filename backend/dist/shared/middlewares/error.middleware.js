...
function scrubHeaders(headers) {
    ...
    for (const [key, value] of Object.entries(headers)) {
        if (ERROR_CONFIG.redactedHeaders.includes(key.toLowerCase())) {
...
function classifyMongooseValidationError(err) {
...
function classifyMongooseCastError(err) {
...
function classifyMongooseDuplicateKey(err) {
...
function classifyMongoNetworkError(err) {
...
function classifySyntaxError(err) {
...
function classifyByStatusCode(err) {
    ...
    const expose = typeof err.expose === "boolean"
        ? err.expose
        : statusCode < 500;
    return {
        statusCode,
...
function deriveCodeFromStatus(status) {
    switch (status) {
...
// LOGGING
// ============================================================
function logError(err, req, classified, requestId, isDev) {
    ...
    if (classified.statusCode >= 500) {
        dbLogger.error("Request failed (5xx): " + JSON.stringify(logPayload));
    }
    else {
        dbLogger.warn("Request failed (4xx): " + JSON.stringify(logPayload));
    }
}
// ============================================================
// ERROR MIDDLEWARE
...
const errorMiddleware = (err, req, res, next) => {
    ...
    const clientMessage = classified.expose
