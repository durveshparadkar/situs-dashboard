...
function shouldSkip(req) {
    ...
    if (RATE_LIMIT_CONFIG.isTest)
        return true;
    // Skip in dev unless explicitly enabled
    if (RATE_LIMIT_CONFIG.isDev && !RATE_LIMIT_CONFIG.enableInDev)
        return true;
    // Skip configured paths (health checks etc.)
    const path = req.originalUrl || req.url || "";
    if (pathMatches(path, RATE_LIMIT_CONFIG.skipPaths))
        return true;
    // Skip bypass roles (SUPER_ADMIN)
    const role = String(req.user?.role ?? "").toUpperCase();
    if (role && RATE_LIMIT_CONFIG.bypassRoles.includes(role)) {
        return true;
    }
    return false;
}
 ...
 */
function buildHandler(limitName) {
    return (req, res) => {
        const userId = req.user?.id ?? req.user?._id ?? "anon";
        const ip = req.ip ?? "unknown";
        const path = req.originalUrl || req.url || "";
        dbLogger.warn("Rate limit exceeded: limiter=" + limitName +
            " user=" + String(userId) +
            " ip=" + ip +
            " path=" + path);
 ...
 */
function buildLimiter(name, overrides) {
    const options = {
 ...
 */
export const passwordLimiter = buildLimiter("password", {
 ...
 */
export const bulkLimiter = buildLimiter("bulk", {
    windowMs: 60 * 1000, // 1 minute
 ...
 */
export const expensiveLimiter = buildLimiter("expensive", {
    windowMs: 60 * 1000, // 1 minute
