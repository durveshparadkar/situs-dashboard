...
const requireAdmin: RequestHandler = (req, res, next) => {
  ...
  if (!(ADMIN_ROLES as readonly string[]).includes(role)) {
    dbLogger.warn(
      `RBAC mutation denied: user=${req.user?.id ?? "anon"} role=${role}`
    );
    res.status(403).json({
      success: false,
      error: {
...
const PERMS = PERMISSIONS as unknown as Record<string, string>;
...
router.get(
  ...
  authorizeMw(READ_RBAC_PERM),
  validateObjectId("id"),
  rbacController.getEffectivePermissions
);
