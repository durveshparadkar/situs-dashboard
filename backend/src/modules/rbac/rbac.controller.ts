...
const RBAC_CONFIG = {
  ...
  privilegedRoles: ["ORG_ADMIN", "SUPER_ADMIN"] as const,
} as const;
...
function requireAuth(req: Request): RbacActor {
  const u = req.user;
  if (!u) {
...
function requireRole(actor: RbacActor, allowed: readonly string[]): void {
  if (!allowed.includes(actor.role)) {
    throw new AppError(
      "RBAC management requires admin-level access",
...
async function validatePermissionIds(
  ...
  const foundIds = found.map((p) => toIdString((p as { _id: unknown })._id));

  if (foundIds.length !== ids.length) {
    const foundSet = new Set(foundIds);
    const missing  = ids.filter((id) => !foundSet.has(id));
    throw new AppError(
      `Invalid permission IDs: ${missing.slice(0, 5).join(", ")}` +
...
async function validateInheritIds(
  ids: string[],
  organizationId: string,
  selfRoleId?: string
): Promise<string[]> {
  ...
  if (found.length !== ids.length) {
    throw new AppError(
      "One or more inherited roles not found or not accessible",
      HttpStatus.BAD_REQUEST,
      "INVALID_INHERITS"
    );
  }

  return found.map((r) => toIdString((r as { _id: unknown })._id));
}
...
class RBACController {
  ...
  /* ── POST /rbac/roles ── */
  createRole = asyncHandler(async (req: Request, res: Response, _next: NextFunction) => {
    ...
    if (existing) {
      throw new AppError(
        `A role named "${validated.name}" already exists`,
        HttpStatus.CONFLICT,
        "ROLE_NAME_EXISTS"
      );
    }

    const validatedPermissions = await validatePermissionIds(
    ...
    dbLogger.info(
      `Role created: id=${String((role as { _id: unknown })._id)} ` +
  ...
  /* ── GET /rbac/roles ── */
  getRoles = asyncHandler(async (req: Request, res: Response, _next: NextFunction) => {
    const actor = requireAuth(req);
    const validated = runSchema(listQuerySchema, req.query);

    const includeSystem =
      validated.includeSystem === undefined
        ? true
  ...
  /* ── GET /rbac/roles/:id ── */
  getRoleById = asyncHandler(async (req: Request, res: Response, _next: NextFunction) => {
    const actor = requireAuth(req);
  ...
  updateRole = asyncHandler(async (req: Request, res: Response, _next: NextFunction) => {
    ...
    if (validatedPermissions  !== undefined) update.permissions = validatedPermissions;
    if (validatedInherits     !== undefined) update.inherits    = validatedInherits;
    ...
    if (!role) {
      throw new AppError(
        "Role not found or not editable (system roles are read-only)",
        HttpStatus.NOT_FOUND,
        "ROLE_NOT_FOUND_OR_LOCKED"
      );
    }

    dbLogger.info(
      `Role updated: id=${id} fields=${Object.keys(update).join(",")} ` +
  ...
  deleteRole = asyncHandler(async (req: Request, res: Response, _next: NextFunction) => {
    const actor = requireAuth(req);
    requireRole(actor, RBAC_CONFIG.privilegedRoles);
    ...
    if (!role) {
      throw new AppError(
        "Role not found or cannot delete system role",
        HttpStatus.NOT_FOUND,
        "ROLE_NOT_FOUND_OR_LOCKED"
      );
    }
    ...
    dbLogger.warn(
      `Role deleted: id=${id} name=${String((role as { name?: string }).name ?? "")} ` +
      `org=${actor.organizationId} actor=${actor.userId}`
    );
  ...
  assignPermissions = asyncHandler(async (req: Request, res: Response, _next: NextFunction) => {
    const actor = requireAuth(req);
    requireRole(actor, RBAC_CONFIG.privilegedRoles);
    ...
    const role = await Role.findOneAndUpdate(
      ...
      { $addToSet: { permissions: { $each: validatedPermissions } } },
      { new: true }
    )
      .populate("permissions")
      .lean();
    ...
    if (!role) {
      throw new AppError(
        "Role not found or not editable",
        HttpStatus.NOT_FOUND,
        "ROLE_NOT_FOUND_OR_LOCKED"
      );
    }
    ...
    dbLogger.info(
      `Permissions assigned: role=${id} count=${validatedPermissions.length} actor=${actor.userId}`
    );
  ...
  revokePermissions = asyncHandler(async (req: Request, res: Response, _next: NextFunction) => {
    const actor = requireAuth(req);
    requireRole(actor, RBAC_CONFIG.privilegedRoles);
    ...
    dbLogger.warn(
      `Permissions revoked: role=${id} count=${permissions.length} actor=${actor.userId}`
    );
  ...
  getPermissions = asyncHandler(async (req: Request, res: Response, _next: NextFunction) => {
    const actor      = requireAuth(req);
    const includeOrg = parseBoolFlag(req.query.includeOrg);

    const filter: Record<string, unknown> = includeOrg
      ? {
          $or: [
            { isSystem: true },
            { organizationId: actor.organizationId },
  ...
  duplicateRole = asyncHandler(async (req: Request, res: Response, _next: NextFunction) => {
    const actor = requireAuth(req);
    requireRole(actor, RBAC_CONFIG.privilegedRoles);
    ...
    const cloneDoc: Record<string, unknown> = {
      name:           newName,
    ...
    dbLogger.info(
      `Role duplicated: source=${id} new=${String((cloned as { _id: unknown })._id)} ` +
  ...
  getEffectivePermissions = asyncHandler(async (req: Request, res: Response, _next: NextFunction) => {
    const actor = requireAuth(req);
    const id    = requireObjectId(req);
    ...
    while (queue.length > 0) {
      ...
      const typed = role as { permissions?: unknown[]; inherits?: unknown[] };
      ...
      for (const r of typed.inherits ?? []) {
        ...
        if (rid && !visited.has(rid)) queue.push(rid);
      }

      if (visited.size > 100) {
        dbLogger.warn(
          `Effective-permission walk exceeded 100 roles: root=${id}`
        );
        break;
      }
    }
