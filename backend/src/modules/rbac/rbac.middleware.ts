...
/**
 * Logical operator for multi-permission checks.
 *   - "all" → user must have EVERY listed permission (default, safer)
...
function toPermissionSet(value: unknown): Set<string> {
  if (value instanceof Set) {
    ...
    for (const item of value) out.add(String(item));
    return out;
  }
...
export function authorize(
  first: string | string[],
  ...rest: Array<string | AuthorizeOptions>
): RequestHandler {

  /* Normalize all call shapes into { permissions, mode } */
  let permissions: string[] = [];
  let mode: AuthorizeMode = "all";
  ...
  const middleware: RequestHandler = async (req, res, next) => {
    try {
      ...
      ===================================================== */
      if ((RBAC_MIDDLEWARE_CONFIG.bypassRoles as readonly string[]).includes(role)) {
        next();
        return;
      }
      ...
      ===================================================== */
      const svc = rbacService as unknown as {
        getUserPermissions: (
          userId: string,
          organizationId: string
        ) => Promise<Set<string> | string[] | readonly string[]>;
      };

      const raw = await withTimeout(
        svc.getUserPermissions(userId, organizationId),
        RBAC_MIDDLEWARE_CONFIG.lookupTimeoutMs,
      ...
      if (!hasAccess) {
        ...
        dbLogger.warn(
          `RBAC deny: user=${userId} org=${organizationId} role=${role} ` +
      ...
      dbLogger.error(
        `RBAC middleware error: ${(err as Error)?.message ?? "unknown"}`
      );
      next(err);
    }
  };
 ...
 */
export function anyPermission(...permissions: string[]): RequestHandler {
 ...
 */
export function allPermissions(...permissions: string[]): RequestHandler {
...
export function requireAnyRole(...roles: string[]): RequestHandler {
  ...
  return (req, res, next) => {
    ...
    if (!allowed.includes(role)) {
      dbLogger.warn(
        `Role gate deny: user=${getUserId(req)} role=${role} required=[${allowed.join(",")}]`
      );
      res.status(403).json({
        success: false,
        error: {
