...
const CONFIG = {
  ...
  skipStatuses:         ["archived", "deleted"] as const,
} as const;
...
async function withTimeout<T>(
  ...
  const timeoutPromise = new Promise<never>((_, reject) => {
    timeoutHandle = setTimeout(
      () => reject(new Error(`${errorMsg} (timeout after ${ms}ms)`)),
      ms
    );
  });
...
async function releaseLock(
  ...
  try {
    ...
    dbLogger.error(
      `[brainWorker] Failed to release lock for lead ${leadId}: ${(err as Error).message}`
    );
  }
}
...
async function processAnalyzeLead(
  job: Job<BrainJobPayload>
  ...
  try {
    await withTimeout(
      analyzeLead(safeLeadId),
      CONFIG.jobTimeoutMs,
      `Brain analysis exceeded timeout for lead ${safeLeadId}`
    );
    ...
    dbLogger.info(
      `[brainWorker:${jobId}] Brain analysis complete: lead=${safeLeadId} ` +
    ...
    dbLogger.error(
      `[brainWorker:${jobId}] Brain processing failed: lead=${safeLeadId} error=${errMsg}`
    );

    await releaseLock(safeLeadId, "failed");

    if (
      err instanceof BrainServiceError &&
...
async function processJob(job: Job<BrainJobPayload>): Promise<ProcessResult | null> {
...
export const brainWorker = new Worker<BrainJobPayload, ProcessResult | null>(
  QUEUE_NAMES.BRAIN,
  processJob,
  {
...
dbLogger.info(
  `[brainWorker] Started: queue=${QUEUE_NAMES.BRAIN} concurrency=${CONFIG.concurrency}`
);
...
brainWorker.on("completed", (job, result) => {
  if (result?.status === "success") {
    dbLogger.info(
      `[brainWorker] Job ${job.id} completed: lead=${result.leadId} ` +
      `priority=${result.priority} durationMs=${result.durationMs}`
    );
  } else if (result?.status === "skipped") {
    dbLogger.info(
      `[brainWorker] Job ${job.id} skipped: lead=${result.leadId} reason=${result.reason}`
    );
  }
});
...
brainWorker.on("failed", (job, err) => {
  ...
  dbLogger.error(
    `[brainWorker] Job ${job?.id} failed (attempt ${attemptsMade}/${maxAttempts}): ${err.message}` +
...
async function shutdown(signal: string): Promise<void> {
  ...
  try {
    ...
    dbLogger.error(`[brainWorker] Shutdown error: ${(err as Error).message}`);
  }
...
process.on("unhandledRejection", (reason) => {
  dbLogger.error(
    `[brainWorker] Unhandled rejection: ${reason instanceof Error ? reason.message : String(reason)}`
  );
});