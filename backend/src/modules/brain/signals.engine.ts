...
type SignalDetector = (context: LeadContext, score: number) => BrainSignal | null;
...
function buildSignal(
  ...
  if (opts.confidence !== undefined) signal.confidence = opts.confidence;
  if (opts.reasoning?.length)        signal.reasoning  = opts.reasoning;
  if (opts.isPositive !== undefined) signal.isPositive = opts.isPositive;
  if (opts.metadata)                 signal.metadata   = opts.metadata;
...
const detectStalled: SignalDetector = (ctx) => {
  const days = ctx.daysSinceLastActivity;
  const cfg  = SIGNALS_CONFIG.inactivity;

  if (days < cfg.stalledDays) return null;

  const isCritical = days >= cfg.extremeStalledDays;
  const severity: PriorityLevel = isCritical ? "critical" : "high";
...
const detectHighValueNoReply: SignalDetector = (ctx) => {
...
const detectFastMoving: SignalDetector = (ctx) => {
  const cfg = SIGNALS_CONFIG.fastMoving;

  if (ctx.activityCount < cfg.minActivities) return null;
  if (!ctx.stageChangedRecently) return null;
  if (ctx.daysSinceLastActivity > cfg.maxInactivityDays) return null;

  return buildSignal(
    "FAST_MOVING",
    "medium",
    "Lead progressing quickly through pipeline",
    {
...
const detectHot: SignalDetector = (ctx, score) => {
  const cfg = SIGNALS_CONFIG.score;

  if (score < cfg.hot) return null;
  if (ctx.daysSinceLastActivity > 3) return null;
  ...
  return buildSignal(
    "HOT",
    isCritical ? "critical" : "high",
    "Highly engaged and active lead",
    {
...
const detectCold: SignalDetector = (ctx, score) => {
  const cfg = SIGNALS_CONFIG.score;
  const inactivityDays = ctx.daysSinceLastActivity;

  if (score > cfg.cold) return null;
  if (inactivityDays < SIGNALS_CONFIG.inactivity.stalledDays) return null;
  ...
  return buildSignal(
    "COLD",
    isHigh ? "high" : "medium",
    "Low engagement and inactive lead",
    {
...
const detectHighRisk: SignalDetector = (ctx) => {
  const value = ctx.dealValue ?? 0;
  const days  = ctx.daysSinceLastActivity;
  ...
  return buildSignal(
    "HIGH_RISK",
    "critical",
    "High-value deal at risk due to inactivity",
    {
...
const detectEnterpriseDeal: SignalDetector = (ctx) => {
  const value = ctx.dealValue ?? 0;
...
const detectReEngaged: SignalDetector = (ctx) => {
  const cfg = SIGNALS_CONFIG.reEngaged;

  if (ctx.daysSinceLastActivity > cfg.maxInactivityDays) return null;
  if (cfg.requiresPrevSignal) {
    const wasInactive = ctx.prevSignals?.some(
      (s) => s === "STALLED" || s === "COLD"
    );
  ...
  return buildSignal(
    "RE_ENGAGED",
    "medium",
    "Lead has re-engaged after period of inactivity",
    {
...
const detectStageStagnation: SignalDetector = (ctx) => {
  const days = ctx.daysInCurrentStage;
  if (days === undefined || days === null) return null;

  const cfg = SIGNALS_CONFIG.stageStagnation;
  let severity: PriorityLevel | null = null;
  let detail = "";
...
const detectCloseDateOverdue: SignalDetector = (ctx) => {
  if (!ctx.expectedCloseDate) return null;

  const close   = new Date(ctx.expectedCloseDate).getTime();
  const now     = Date.now();
  const daysOut = (close - now) / (1000 * 60 * 60 * 24);

  if (daysOut >= 0) return null; // not overdue

  const overdueDays = Math.floor(-daysOut);
  const severity: PriorityLevel = overdueDays > 14 ? "critical" : "high";

  return buildSignal(
    "CLOSE_DATE_OVERDUE",
    severity,
    `Expected close date passed ${overdueDays} day(s) ago`,
    {
...
const detectCloseDateAtRisk: SignalDetector = (ctx) => {
  if (!ctx.expectedCloseDate) return null;
  ...
  if (daysOut < 0 || daysOut > cfg.imminentDays) return null;
  ...
  if (!isShaky) return null;

  return buildSignal(
    "CLOSE_DATE_AT_RISK",
    "high",
    `Closing in ${Math.floor(daysOut)} day(s) but momentum is weak`,
    {
...
const ALL_DETECTORS: ReadonlyArray<{
  name: string;
