...
export class InsightServiceError extends Error {
  constructor(
    ...
    public code: string = "INSIGHT_SERVICE_ERROR"
  ) {
    super(message);
...
export interface CreateInsightInput {
  ...
  expiresAt?: Date;
  fingerprint?: string;            // optional — auto-generated if not provided
...
export interface InsightFilters {
  ...
  assignedTo?: string;
  targetType?: IInsightTarget["type"];
  targetId?: string;
  source?: InsightSource;
  generatedAfter?: Date;
  generatedBefore?: Date;
  sortBy?: "severity" | "generatedAt" | "confidence";
  sortOrder?: "asc" | "desc";
}
...
export interface InsightStats {
  ...
  acceptedCount: number;          // marked as acted
  dismissedCount: number;
  acceptanceRate: number;          // acted / (acted + dismissed)
}
...
class InsightService {
  ...
  ===================================================== */
  async createInsight(input: CreateInsightInput): Promise<InsightDocument> {
    /* ── Validation ── */
  ...
  ...(input.target.url !== undefined && { url: input.target.url }),
    ...
    dbLogger.info(
      `Insight upserted: org=${input.organizationId} type=${input.type} ` +
  ...
  ===================================================== */
  async bulkCreateInsights(inputs: CreateInsightInput[]): Promise<{
    created: number;
    updated: number;
    failed: number;
  }> {
    if (!inputs.length) return { created: 0, updated: 0, failed: 0 };
    ...
    for (let i = 0; i < inputs.length; i += INSIGHT_CONFIG.bulkChunkSize) {
      ...
      for (const r of results) {
        if (r.status === "fulfilled") {
          ...
          failed++;
          dbLogger.error(
            `Bulk insight create failure: ${(r.reason as Error)?.message}`
          );
        }
      }
    }
    ...
    dbLogger.info(
      `Bulk insights complete: total=${inputs.length} ` +
  ...
  async listInsights(
    ...
    const limit = Math.min(
      Math.max(filters.limit ?? INSIGHT_CONFIG.defaultPageSize, 1),
      INSIGHT_CONFIG.maxPageSize
    );
    ...
    if (severity?.length) query.severity = { $in: severity };
    ...
    if (filters.targetType) {
      (query as any)["target.type"] = filters.targetType;
    }

    if (filters.targetId && isValidObjectId(filters.targetId)) {
      (query as any)["target.id"] = toObjectId(filters.targetId);
    }
    ...
    if (filters.generatedAfter || filters.generatedBefore) {
      ...
      if (filters.generatedAfter) dateFilter.$gte = filters.generatedAfter;
      if (filters.generatedBefore) dateFilter.$lte = filters.generatedBefore;
  ...
  async getInsightsForRep(
    ...
    return Insight.findActiveForUser(
      toObjectId(userId),
  ...
  async dismiss(
    ...
    await insight.save();
    dbLogger.info(
      `Insight dismissed: id=${insightId} user=${userId} reason=${reason ?? "none"}`
    );
    return insight;
  }
  ...
  async snooze(
    ...
    await insight.save();
    dbLogger.info(
      `Insight snoozed: id=${insightId} until=${until.toISOString()}`
    );
    return insight;
  }
  ...
  submittedBy: toObjectId(userId),
    ...
    dbLogger.info(
      `Insight feedback submitted: id=${insightId} rating=${feedback.rating}`
    );
  ...
  ===================================================== */
  async getStats(organizationId: string): Promise<InsightStats> {
    if (!isValidObjectId(organizationId)) {
      throw new InsightServiceError(
        "Invalid organization ID",
        400,
    ...
    const acceptanceRate =
      accDenom > 0 ? (accData.acted / accDenom) * 100 : 0;
  ...
  async expireStale(organizationId: string): Promise<number> {
    ...
    dbLogger.info(
      `Stale insights expired: org=${organizationId} count=${count}`
    );
  ...
  async autoResolveByTarget(
    ...
    if (result.modifiedCount > 0) {
      dbLogger.info(
        `Auto-resolved ${result.modifiedCount} insights: ` +
