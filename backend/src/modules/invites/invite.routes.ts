    ...
    try {
      ...
      if (err instanceof RateLimiterRes) {
        ...
        res.set({
          ...
          "X-RateLimit-Reset":     new Date(
            Date.now() + err.msBeforeNext
          ).toISOString(),
        });

        dbLogger.warn(
          `Rate limit exceeded: limiter=${options.name} ` +
      ...
      dbLogger.error(
        `Rate limiter ${options.name} failed (failing open): ` +
...
router.post(
  "/",
  authorizeMw("CREATE_INVITE"),
  requireActiveBillingMw,
  rateLimitMiddleware(createInviteLimiter, {
    keyFrom: "user",
    name:    "create_invite",
  }),
  createInvite
...
router.post(
  "/accept",
  rateLimitMiddleware(acceptInviteLimiter, {
    keyFrom: "ip",
    name:    "accept_invite",
  }),
  acceptInvite
...
router.post(
  ...
  requireActiveBillingMw,
  validateObjectId("id"),
  rateLimitMiddleware(resendInviteLimiter, {
    keyFrom: "user",
    name:    "resend_invite",
  }),
  resendInvite
