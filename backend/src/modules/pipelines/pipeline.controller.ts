 ...
 */
function requireAuth(req: Request): PipelineActor {
  const u = req.user;
  if (!u) {
...
function requireRole(actor: PipelineActor, allowed: readonly string[]): void {
 ...
 */
function validateStageBusinessRules(
  ...
  for (let i = 0; i < stages.length; i++) {
    ...
    if (stage.isWon && stage.isLost) {
      throw new AppError(
        `Stage at index ${i} cannot be both won and lost`,
        HttpStatus.BAD_REQUEST,
        "INVALID_STAGE"
      );
    }
  ...
  if (wonCount > 1) {
    throw new AppError(
      "A pipeline can have at most one 'won' stage",
      HttpStatus.BAD_REQUEST,
      "MULTIPLE_WON_STAGES"
    );
  }
  if (lostCount > 1) {
    throw new AppError(
      "A pipeline can have at most one 'lost' stage",
      HttpStatus.BAD_REQUEST,
      "MULTIPLE_LOST_STAGES"
    );
  }
}
...
class PipelineController {
  ...
  create = asyncHandler(async (req: Request, res: Response, _next: NextFunction) => {
    ...
    const svc = pipelineService as unknown as {
    ...
    dbLogger.info(
      `Pipeline created: org=${actor.organizationId} ` +
  ...
  getAll = asyncHandler(async (req: Request, res: Response, _next: NextFunction) => {
    ...
    const limit = getNumber(
      validated.limit,
      PIPELINE_CONFIG.pagination.defaultLimit,
      { min: 1, max: PIPELINE_CONFIG.pagination.maxLimit }
    );

    const search = validated.search?.trim().slice(0, 200);

    const svc = pipelineService as unknown as {
      getPipelinesByOrganization: (
        organizationId: string,
        page: number,
        limit: number,
        opts?: { search?: string }
    ...
    const result = await svc.getPipelinesByOrganization(
  ...
  ===================================================== */
  getDefault = asyncHandler(async (req: Request, res: Response, _next: NextFunction) => {
    const actor = requireAuth(req);
    ...
    if (!pipeline) {
      throw new AppError(
        "No default pipeline configured for this organization",
  ...
  update = asyncHandler(async (req: Request, res: Response, _next: NextFunction) => {
    ...
    dbLogger.info(
      `Pipeline updated: id=${id} org=${actor.organizationId} ` +
  ...
  remove = asyncHandler(async (req: Request, res: Response, _next: NextFunction) => {
    ...
    const svc = pipelineService as unknown as {
      getPipelineById?: (
        id: string,
        organizationId: string
      ) => Promise<{ isDefault?: boolean } | null>;
      deletePipeline: (id: string, organizationId: string) => Promise<unknown>;
    };
    ...
    dbLogger.warn(
      `Pipeline deleted: id=${id} org=${actor.organizationId} actor=${actor.userId}`
    );
  ...
  ===================================================== */
  setDefault = asyncHandler(async (req: Request, res: Response, _next: NextFunction) => {
    const actor = requireAuth(req);
    requireRole(actor, PIPELINE_CONFIG.privilegedRoles);
    ...
    if (typeof svc.setDefaultPipeline === "function") {
      ...
      result = await svc.updatePipeline(
        id,
        { isDefault: true },
        actor.organizationId
      );
    ...
    dbLogger.info(
      `Pipeline set as default: id=${id} org=${actor.organizationId} actor=${actor.userId}`
    );
  ...
  duplicate = asyncHandler(async (req: Request, res: Response, _next: NextFunction) => {
    ...
    const svc = pipelineService as unknown as {
      duplicatePipeline?: (
        id: string,
        organizationId: string,
        opts: { newName?: string; actorId: string }
      ) => Promise<unknown>;
    };
    ...
    dbLogger.info(
      `Pipeline duplicated: source=${id} org=${actor.organizationId} actor=${actor.userId}`
    );
