...
export class EntityServiceError extends Error {
  constructor(
    ...
    public code: string = "ENTITY_SERVICE_ERROR"
  ) {
    super(message);
...
function normalizeFilterArray<T>(value: T | T[] | undefined): T[] | undefined {
...
class EntityService {
  ...
  /* =====================================================
     CREATE
  ===================================================== */
  async createEntity(payload: CreateEntityPayload): Promise<EntityDocument> {
    /* ── Validation ── */
    ...
    if (payload.type && !(ENTITY_TYPES as readonly string[]).includes(payload.type)) {
      throw new EntityServiceError(
        `Invalid type. Must be one of: ${ENTITY_TYPES.join(", ")}`,
        400,
        "INVALID_TYPE"
      );
    }

    if (payload.status && !(ENTITY_STATUSES as readonly string[]).includes(payload.status)) {
      throw new EntityServiceError(
        `Invalid status. Must be one of: ${ENTITY_STATUSES.join(", ")}`,
        400,
        "INVALID_STATUS"
      );
    }
    ...
    dbLogger.info(
      `Entity created: org=${payload.organizationId} ` +
  ...
  async listEntities(
    ...
    if (filters.tags && filters.tags.length > 0) {
      const cleanTags = filters.tags
        ...
        .filter(Boolean);
      if (cleanTags.length) filter.tags = { $in: cleanTags };
    }
    ...
    const sortField: SortableField =
      filters.sortBy && (SORTABLE_FIELDS as readonly string[]).includes(filters.sortBy)
        ? filters.sortBy
        : "createdAt";
...
const sort: Record<string, 1 | -1> = { [sortField]: sortDir };
  ...
  async getEntityById(
    ...
    if (opts.recordView) {
      entity.recordView().catch((err) => {
        dbLogger.error(
          `View tracking failed for entity ${entityId}: ${(err as Error).message}`
        );
      });
    }
  ...
  async updateEntity(
    ...
    if (updates.type && !(ENTITY_TYPES as readonly string[]).includes(updates.type)) {
      throw new EntityServiceError("Invalid type", 400, "INVALID_TYPE");
    }
    if (updates.status && !(ENTITY_STATUSES as readonly string[]).includes(updates.status)) {
      throw new EntityServiceError("Invalid status", 400, "INVALID_STATUS");
    }
    if (
      updates.visibility &&
      !(ENTITY_VISIBILITY as readonly string[]).includes(updates.visibility)
    ) {
    ...
    const $set: Record<string, unknown> = { updatedBy: toObjectId(userId) };

    if (updates.title !== undefined)       $set.title       = updates.title.trim().slice(0, 300);
    if (updates.description !== undefined) $set.description = updates.description.slice(0, 5000);
    ...
    if (updates.ownerId !== undefined)     $set.ownerId     = toObjectId(updates.ownerId);
    if (updates.externalIds !== undefined) $set.externalIds  = updates.externalIds;
    ...
    dbLogger.info(
      `Entity updated: org=${organizationId} id=${entityId} ` +
  ...
  async softDeleteEntity(
    ...
    dbLogger.warn(
      `Entity soft-deleted: org=${organizationId} id=${entityId} user=${userId}`
    );
  ...
  async restoreEntity(
    ...
    if (!entity) {
      throw new EntityServiceError(
        "Entity not found or not deleted",
        404,
        "ENTITY_NOT_FOUND"
      );
    }
    ...
    dbLogger.info(
      `Entity restored: org=${organizationId} id=${entityId} user=${userId}`
    );
  ...
  ===================================================== */
  async bulkCreateEntities(payloads: CreateEntityPayload[]): Promise<{
    created: number;
    failed: number;
    errors: Array<{ index: number; error: string }>;
  }> {
    if (!payloads.length) return { created: 0, failed: 0, errors: [] };
    ...
    for (let i = 0; i < payloads.length; i += ENTITY_CONFIG.bulkChunkSize) {
      ...
      const results = await Promise.allSettled(
        chunk.map((p) => this.createEntity(p))
      );
    ...
    dbLogger.info(
      `Bulk entity create: total=${payloads.length} created=${created} failed=${failed}`
    );
  ...
  ===================================================== */
  async getStats(organizationId: string): Promise<EntityStats> {
    if (!isValidObjectId(organizationId)) {
      throw new EntityServiceError(
        "Invalid organization ID",
        400,
  ...
  async findByExternalId(
    ...
    return Entity.findByExternalId(
  ...
  async hardDeleteEntity(
    ...
    if (result.deletedCount === 0) {
      throw new EntityServiceError(
        "Entity not found or not soft-deleted (hard delete requires prior soft delete)",
        404,
        "ENTITY_NOT_FOUND"
      );
    }
    ...
    dbLogger.warn(
      `Entity HARD DELETED: org=${organizationId} id=${entityId}`
    );
