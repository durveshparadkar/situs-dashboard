...
type AllowedSortField = (typeof ENTITY_CONFIG.allowedSortFields)[number];
 ...
 */
function requireAuth(req: Request): EntityActor {
  const u = req.user;
  if (!u) {
 ...
 */
function parseSortField(sort: string | undefined): Record<string, 1 | -1> {
  const raw =
    sort && sort.trim().length > 0 ? sort.trim() : ENTITY_CONFIG.defaultSort;
  const direction: 1 | -1 = raw.startsWith("-") ? -1 : 1;
  const field = raw.replace(/^-/, "");

  if (!(ENTITY_CONFIG.allowedSortFields as readonly string[]).includes(field)) {
    throw new AppError(
      `Invalid sort field. Allowed: ${ENTITY_CONFIG.allowedSortFields.join(", ")}`,
      HttpStatus.BAD_REQUEST,
      "INVALID_SORT_FIELD"
    );
  }

  return { [field as AllowedSortField]: direction };
}
...
export const createEntity = async (
  ...
  try {
    ...
    dbLogger.info(
      `Entity created: id=${entity._id} type=${entity.type} ` +
...
export const getEntities = async (
  ...
  try {
    ...
    const limitRaw = parseInt(
      validatedQuery.limit ?? String(ENTITY_CONFIG.defaultPageSize),
      10
    );
    ...
    const limit = Math.min(
      Math.max(
        Number.isFinite(limitRaw) ? limitRaw : ENTITY_CONFIG.defaultPageSize,
        1
      ),
      ENTITY_CONFIG.maxPageSize
    );
...
export const getEntityById = async (
  ...
  try {
    ...
    Entity.updateOne(
      ...
      dbLogger.warn(
        `Failed to record entity view: id=${id} error=${(err as Error)?.message ?? "unknown"}`
      );
    });
...
export const updateEntity = async (
  ...
  try {
    ...
    dbLogger.info(
      `Entity updated: id=${id} org=${organizationId} user=${userId} ` +
...
export const deleteEntity = async (
  ...
  try {
    ...
    dbLogger.warn(
      `Entity deleted: id=${id} org=${organizationId} user=${userId}`
    );
...
export const restoreEntity = async (
  ...
  try {
    ...
    const entity = await Entity.findOneAndUpdate(
      { _id: id, organizationId, isDeleted: true },
      {
        $set: {
          isDeleted:  false,
          deletedAt:  null,
    ...
    dbLogger.info(
      `Entity restored: id=${id} org=${organizationId} user=${userId}`
    );
...
export const bulkCreateEntities = async (
  ...
  try {
    ...
    dbLogger.info(
      `Entity bulk create: org=${organizationId} user=${userId} ` +
...
export const findEntityByExternalId = async (
  ...
  try {
    ...
    if (!source || !externalId) {
      throw new AppError(
        "Both 'source' and 'externalId' query params are required",
        HttpStatus.BAD_REQUEST,
        "MISSING_PARAMS"
      );
    }
    ...
    if (!["hubspot", "salesforce"].includes(source.toLowerCase())) {
      throw new AppError(
        "source must be 'hubspot' or 'salesforce'",
        HttpStatus.BAD_REQUEST,
        "INVALID_SOURCE"
      );
    }
...
const entity = await Entity.findOne({
  ...
  [fieldKey]: externalId,
    ...
    if (!entity) {
      throw new AppError(
        "Entity not found for that external ID",
        HttpStatus.NOT_FOUND,
        "ENTITY_NOT_FOUND"
      );
    }
