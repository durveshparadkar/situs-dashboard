...
const requireManager: RequestHandler = (req, res, next) => {
  ...
  if (!(MANAGER_ROLES as readonly string[]).includes(role)) {
    dbLogger.warn(
      `User-mgmt denied: user=${req.user?.id ?? "anon"} role=${role}`
    );
    res.status(403).json({
      success: false,
      error: {
...
const requireAdmin: RequestHandler = (req, res, next) => {
  ...
  if (!(ADMIN_ROLES as readonly string[]).includes(role)) {
    dbLogger.warn(
      `User-admin action denied: user=${req.user?.id ?? "anon"} role=${role}`
    );
    res.status(403).json({
      success: false,
      error: {
...
const checkUserLimitMw  = checkUserLimit  as unknown as RequestHandler;
