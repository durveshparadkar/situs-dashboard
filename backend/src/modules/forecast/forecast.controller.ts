...
const VALID_RANGES = ["week", "month", "quarter", "year", "custom"] as const;
type ForecastRange = (typeof VALID_RANGES)[number];

const VALID_GROUPBY = ["stage", "owner", "month", "week"] as const;
type ForecastGroupBy = (typeof VALID_GROUPBY)[number];
...
type ForecastModel = (typeof VALID_MODELS)[number];

/* =====================================================
   HELPERS
===================================================== */

interface ForecastActor {
  userId: string;
 ...
 */
function requireAuth(req: Request): ForecastActor {
  const u = req.user;
  if (!u) {
...
function validateCustomRange(
  ...
  if (!start || !end) {
    throw new AppError(
      "startDate and endDate are required when range='custom'",
  ...
  if (spanMs > maxMs) {
    throw new AppError(
      `Date range cannot exceed ${FORECAST_CONFIG.maxRangeDays} days`,
      HttpStatus.BAD_REQUEST,
      "RANGE_TOO_LARGE"
    );
  }
 ...
 */
function runForecastValidator(query: unknown): ForecastQueryInput {
  try {
    const result = validateForecastQuery(query);
    return (result ?? query) as ForecastQueryInput;
  } catch (err) {
    const issues =
    ...
    throw new AppError(
      (err as Error)?.message || "Invalid forecast query",
      HttpStatus.BAD_REQUEST,
      "VALIDATION_ERROR"
    );
  }
 ...
 */
function setForecastCacheHeaders(res: Response): void {
  res.setHeader(
    "Cache-Control",
    `private, max-age=${FORECAST_CONFIG.cacheControlSeconds}`
  );
}
...
class ForecastController {
  ...
  async getForecast(
    ...
    try {
      ...
      const range =
        getEnumParam(parsedQuery.range, VALID_RANGES, "month") ?? "month";

      const model =
        getEnumParam(parsedQuery.model, VALID_MODELS, "weighted_pipeline") ??
        "weighted_pipeline";

      /* ── Custom range validation ── */
      let dateWindow: { start: Date; end: Date } | undefined;
      if (range === "custom") {
        dateWindow = validateCustomRange(
          getDateParam(parsedQuery.startDate),
          getDateParam(parsedQuery.endDate)
        );
      }
      ...
      dbLogger.info(
        `Forecast requested: org=${organizationId} user=${userId} ` +
      ...
      const svc = ForecastService as unknown as {
        getForecast: (
          orgId: string,
          range?: string,
          opts?: {
      ...
      const data = await withTimeout(
        Promise.resolve(
          svc.getForecast(organizationId, range, {
            model,
            requestedBy: userId,
            role,
            ...(ownerId    && { ownerId }),
      ...
      if (durationMs > FORECAST_CONFIG.slowMs) {
        dbLogger.warn(
          `Slow forecast: org=${organizationId} range=${range} ` +
          `model=${model} durationMs=${durationMs}`
        );
      }

      dbLogger.info(
        `Forecast complete: org=${organizationId} durationMs=${durationMs}`
      );
  ...
  async getForecastSummary(
    ...
    try {
      ...
      const range =
        getEnumParam(req.query.range, VALID_RANGES, "month") ?? "month";
      ...
      const svc = ForecastService as unknown as {
        ...
        getForecast: (orgId: string, range?: string) => Promise<unknown>;
      };

      const startedAt = Date.now();

      const result =
        typeof svc.getForecastSummary === "function"
          ? await withTimeout(
              Promise.resolve(
                svc.getForecastSummary(organizationId, range, {
                  requestedBy: userId,
                })
              ),
              20_000,
          ...
          : await withTimeout(
              Promise.resolve(svc.getForecast(organizationId, range)),
              20_000,
      ...
      dbLogger.info(
        `Forecast summary: org=${organizationId} range=${range} ` +
  ...
  async getForecastBreakdown(
    ...
    try {
      ...
      const groupBy =
        getEnumParam(req.query.groupBy, VALID_GROUPBY, "stage") ?? "stage";
      ...
      const svc = ForecastService as unknown as {
        getForecastBreakdown?: (
          ...
          opts?: { range?: string; ownerId?: string; requestedBy?: string }
        ) => Promise<unknown>;
        getForecast: (orgId: string, range?: string) => Promise<unknown>;
      };
      ...
      const result =
        typeof svc.getForecastBreakdown === "function"
          ? await withTimeout(
              Promise.resolve(
                svc.getForecastBreakdown(organizationId, groupBy, {
                  range,
                  requestedBy: userId,
                  ...(ownerId && { ownerId }),
                })
      ...
      if (durationMs > FORECAST_CONFIG.slowMs) {
        dbLogger.warn(
          `Slow forecast breakdown: org=${organizationId} ` +
          `groupBy=${groupBy} range=${range} durationMs=${durationMs}`
        );
      }

      dbLogger.info(
        `Forecast breakdown: org=${organizationId} groupBy=${groupBy} ` +
        `durationMs=${durationMs}`
      );
  ...
  async getForecastAccuracy(
    ...
    try {
      ...
      if (!["ORG_ADMIN", "SUPER_ADMIN", "MANAGER"].includes(role)) {
        throw new AppError(
          "Accuracy report restricted to managers",
      ...
      const svc = ForecastService as unknown as {
        getForecastAccuracy?: (
          orgId: string,
          opts: { lookbackMonths: number; requestedBy: string }
        ) => Promise<unknown>;
      };

      if (typeof svc.getForecastAccuracy !== "function") {
        throw new AppError(
          "Accuracy reporting not available",
      ...
      dbLogger.info(
        `Forecast accuracy: org=${organizationId} months=${lookbackMonths} ` +
        `user=${userId}`
      );
  ...
  async refreshForecast(
    ...
    try {
      ...
      if (!["ORG_ADMIN", "SUPER_ADMIN", "MANAGER"].includes(role)) {
        throw new AppError(
          "Refresh restricted to managers",
          HttpStatus.FORBIDDEN,
          "FORBIDDEN"
        );
      }
      ...
      const svc = ForecastService as unknown as {
        refreshForecast?: (
          orgId: string,
          opts: { requestedBy: string; force: true }
        ) => Promise<unknown>;
        getForecast: (
          orgId: string,
          range?: string,
      ...
      const data = typeof svc.refreshForecast === "function"
        ? await withTimeout(
            Promise.resolve(
              svc.refreshForecast(organizationId, {
                requestedBy: userId,
                force: true,
              })
            ),
        ...
        : await withTimeout(
            Promise.resolve(
              svc.getForecast(organizationId, "month", {
                force: true,
                requestedBy: userId,
              })
            ),
            60_000,
            "Forecast refresh (fallback)"
          );

      const durationMs = Date.now() - startedAt;

      dbLogger.warn(
        `Forecast refresh: org=${organizationId} user=${userId} ` +
        `durationMs=${durationMs}`
      );
