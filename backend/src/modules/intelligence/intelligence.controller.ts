...
function requireAuth(req: Request): {
  ...
  const effectiveOrganizationId =
    req.effectiveOrganizationId ?? req.user.organizationId;
 ...
 */
function parseInput<T>(schema: z.ZodType<T>, data: unknown, source: string): T {
  try {
 ...
 */
function validateObjectId(id: unknown, field: string): string {
...
class IntelligenceController {
  ...
  refresh: RequestHandler = asyncHandler(
    async (req: Request, res: Response, _next: NextFunction): Promise<void> => {
      const auth = requireAuth(req);
      const body = parseInput(refreshBodySchema, req.body ?? {}, "refresh body");

      const runInput: Parameters<typeof intelligenceService.run>[0] = {
  ...
  preview: RequestHandler = asyncHandler(
    async (req: Request, res: Response, _next: NextFunction): Promise<void> => {
      const auth = requireAuth(req);
      const body = parseInput(previewBodySchema, req.body ?? {}, "preview body");

      const previewInput: Parameters<typeof intelligenceService.preview>[0] = {
        organizationId: auth.effectiveOrganizationId,
        actorId:        auth.userId,
      };
  ...
  refreshDeal: RequestHandler = asyncHandler(
    async (req: Request, res: Response, _next: NextFunction): Promise<void> => {
      const auth = requireAuth(req);
      const dealId = validateObjectId(req.params.dealId, "dealId");

      const result = await intelligenceService.refreshOneDeal(
        dealId,
        auth.effectiveOrganizationId,
        auth.userId
      );
  ...
  summary: RequestHandler = asyncHandler(
    async (req: Request, res: Response, _next: NextFunction): Promise<void> => {
      const auth = requireAuth(req);
  ...
  forecast: RequestHandler = asyncHandler(
    async (req: Request, res: Response, _next: NextFunction): Promise<void> => {
      const auth = requireAuth(req);
      const query = parseInput(summaryQuerySchema, req.query, "query");
  ...
  leaks: RequestHandler = asyncHandler(
    async (req: Request, res: Response, _next: NextFunction): Promise<void> => {
      const auth = requireAuth(req);
      const query = parseInput(summaryQuerySchema, req.query, "query");
  ...
  actions: RequestHandler = asyncHandler(
    async (req: Request, res: Response, _next: NextFunction): Promise<void> => {
      const auth = requireAuth(req);
      const query = parseInput(summaryQuerySchema, req.query, "query");
