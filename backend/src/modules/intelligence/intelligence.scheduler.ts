...
/**
 * Parent batch job — enumerates orgs, kicks off per-org runs.
 */
interface IntelligenceBatchJobData {
  triggeredBy?: "schedule" | "manual";
  scheduledAt: string;
}

interface IntelligenceBatchJobResult {
...
let intelligenceQueue: Queue<IntelligenceBatchJobData, IntelligenceBatchJobResult> | null = null;
let intelligenceWorker: Worker<IntelligenceBatchJobData, IntelligenceBatchJobResult> | null = null;
let intelligenceQueueEvents: QueueEvents | null = null;

/**
 * Lazily get or build the queue. Single instance per process.
 */
function getQueue(): Queue<IntelligenceBatchJobData, IntelligenceBatchJobResult> {
  if (!intelligenceQueue) {
    intelligenceQueue = new Queue<IntelligenceBatchJobData, IntelligenceBatchJobResult>(
      SCHEDULER_CONFIG.queueName,
      {
        connection,
        defaultJobOptions: {
    ...
    queueLogger.info(
      "Intelligence queue initialized: name=" + SCHEDULER_CONFIG.queueName
    );
  }
 ...
 */
async function loadActiveOrganizations(): Promise<Types.ObjectId[]> {
...
async function processOneOrg(
  ...
  const timeoutPromise = new Promise<never>((_resolve, reject) => {
    setTimeout(
      () => reject(new Error("Per-org timeout exceeded")),
      SCHEDULER_CONFIG.perOrgTimeoutMs
    );
  });
...
async function processBatchJob(
  job: Job<IntelligenceBatchJobData>
): Promise<IntelligenceBatchJobResult> {
  const batchStart = Date.now();
  queueLogger.info(
    "Intelligence batch starting: " +
  ...
  for (let i = 0; i < orgIds.length; i += concurrency) {
    ...
    for (const { orgId, result } of results) {
      if (result.ok) {
        orgsSucceeded += 1;
        engineLogger.debug(
          "Intelligence run ok: org=" + String(orgId) +
        ...
        engineLogger.warn(
          "Intelligence run failed: org=" + String(orgId) +
          " error=" + result.error
        );
      }
    }
 ...
 */
export function startIntelligenceWorker(): Worker<IntelligenceBatchJobData, IntelligenceBatchJobResult> | null {
  if (!SCHEDULER_CONFIG.enabled) {
    queueLogger.info("Intelligence scheduler disabled (test or env-disabled)");
    return null;
  }

  if (intelligenceWorker) return intelligenceWorker;

  intelligenceWorker = new Worker<IntelligenceBatchJobData, IntelligenceBatchJobResult>(
    SCHEDULER_CONFIG.queueName,
    processBatchJob,
    {
      connection,
  ...
  intelligenceWorker.on("completed", (job, result) => {
    queueLogger.info(
      "Intelligence batch completed: jobId=" + job.id +
  ...
  intelligenceWorker.on("failed", (job, err) => {
    dbLogger.error(
      "Intelligence batch failed: jobId=" + (job?.id ?? "unknown") +
  ...
  intelligenceWorker.on("error", (err) => {
    dbLogger.error(
      "Intelligence worker error: " + (err?.message ?? "unknown")
    );
  });
 ...
 */
export async function scheduleIntelligenceNightly(): Promise<void> {
  if (!SCHEDULER_CONFIG.enabled) return;
  ...
  queueLogger.info(
    "Intelligence nightly scheduled: " +
 ...
 */
export async function triggerIntelligenceNow(
  actorId?: string
): Promise<string> {
  const queue = getQueue();

  const job = await queue.add(
    "intelligence-batch-manual",
    {
      triggeredBy: "manual",
      scheduledAt: new Date().toISOString(),
    },
  ...
  queueLogger.info(
    "Intelligence batch triggered manually: " +
 ...
 */
export async function closeIntelligenceWorker(): Promise<void> {
...
export {
  SCHEDULER_CONFIG as INTELLIGENCE_SCHEDULER_CONFIG,
  getQueue as getIntelligenceQueue,
};