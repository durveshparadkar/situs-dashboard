...
function parseRedisUrl(url: string | undefined): ConnectionOptions {
  ...
  try {
    ...
    if (parsed.protocol === "rediss:") {
      (config as { tls?: object }).tls = {};
    }
    return config;
  } catch (err) {
    dbLogger.error(
      "Scoring scheduler: invalid REDIS_URL, using localhost: " +
...
interface OrgJobData {
  kind: "org";
  organizationId: string;
}

type ScoringJobData = SweepJobData | OrgJobData;

/* =====================================================
   QUEUE
===================================================== */

export const scoringQueue = new Queue<ScoringJobData>(
...
export const scoringQueueEvents = new QueueEvents(
  SCHEDULER_CONFIG.queueName,
  { connection }
);
 ...
 */
export async function enqueueOrgScoring(
  organizationId: string
): Promise<string> {
  ...
  dbLogger.info(
    "Scoring enqueued: org=" + organizationId + " jobId=" + (job.id ?? "")
  );
 ...
 * server boot.
 */
export async function registerScoringSweep(): Promise<void> {
  await scoringQueue.add(
    SCHEDULER_CONFIG.sweepJobName,
    { kind: "sweep" },
    {
  ...
  dbLogger.info(
    "Scoring sweep registered: cron=" + SCHEDULER_CONFIG.sweepCron
  );
}
 ...
 */
async function findOrgsWithOpenDeals(): Promise<string[]> {
...
async function processScoringJob(job: Job<ScoringJobData>): Promise<void> {
  ...
  if (data.kind === "org") {
    ...
    dbLogger.info(
      "Scoring job done (org): org=" + data.organizationId +
  ...
  for (const orgId of orgIds) {
    try {
      ...
      totalFailed++;
      dbLogger.error(
        "Scoring sweep: org failed org=" + orgId +
  ...
  dbLogger.info(
    "Scoring sweep complete: orgs=" + orgIds.length +
...
scoringWorker.on("failed", (job, err) => {
  dbLogger.error(
    "Scoring job failed: jobId=" + (job?.id ?? "") +
...
scoringWorker.on("error", (err) => {
  dbLogger.error(
    "Scoring worker error: " + ((err as Error)?.message ?? "unknown")
  );
});

scoringQueue.on("error", (err) => {
  dbLogger.error(
    "Scoring queue error: " + ((err as Error)?.message ?? "unknown")
  );
});
 ...
 */
export async function closeScoringScheduler(): Promise<void> {
  ...
  try {
    ...
    dbLogger.error(
      "Error closing scoring scheduler: " +
      ((err as Error)?.message ?? "unknown")
    );
    throw err;
  }
