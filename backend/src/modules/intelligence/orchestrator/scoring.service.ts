...
import Deal, { type DealDocument, type RiskLevel } from "../../deals/deal.model.js";
 ...
 */
function toEngineDeal(deal: DealDocument): {
  name: string;
  value: number;
  stage: string;
...
export interface DealScoreOutcome {
  dealId: string;
  title: string;
  riskScore: number;
  riskLevel: RiskLevel;
...
class ScoringService {
  ...
  async scoreDeal(
    ...
    deal.riskFactors = result.reasons.map((reason: string) => ({
      factor:     reason,
      weight:     0,
      detectedAt: new Date(),
      resolved:   false,
   ...
   */
  async scoreOrganization(organizationId: string): Promise<ScoringRunSummary> {
    ...
    const outcomes: DealScoreOutcome[] = [];
    let failed = 0;
    ...
    for (const deal of deals) {
      try {
        ...
        failed++;
        dbLogger.error(
          "Scoring failed for deal=" + String(deal._id) +
          " org=" + organizationId +
    ...
    dbLogger.info(
      "Scoring run complete: org=" + organizationId +
