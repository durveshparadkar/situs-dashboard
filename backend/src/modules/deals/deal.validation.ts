...
function runSchema<T extends z.ZodTypeAny>(
  ...
  if (!result.success) {
    const details = result.error.issues.map((e: z.ZodIssue) => ({
...
export const validateCreateDeal = (data: unknown): CreateDealInput =>
  runSchema(createDealSchema, data);

export const validateUpdateDeal = (data: unknown): UpdateDealInput =>
  runSchema(updateDealSchema, data);

export const validateUpdateStage = (data: unknown): UpdateStageInput =>
  runSchema(updateStageSchema, data);

export const validateDealFilters = (data: unknown): DealFiltersInput =>
  runSchema(dealFiltersSchema, data);

export const validateBulkUpdateStage = (data: unknown): BulkUpdateStageInput =>
  runSchema(bulkUpdateStageSchema, data);
