...
const SUBSCRIPTION_CONFIG = {
  ...
  billingAdminRoles:   ["ORG_ADMIN", "SUPER_ADMIN"] as const,
  invoiceListLimit:    10,
...
const PAID_PLANS: readonly PaidPlan[] = ["SMALL_BUSINESS", "PRO"];
...
const planSchema = z.enum(PAID_PLANS as unknown as [PaidPlan, ...PaidPlan[]]);
...
function requireAuth(req: Request): BillingActor {
  const u = req.user;
  if (!u) {
  ...
  const email = typeof (u as { email?: unknown }).email === "string"
...
function requireBillingAdmin(actor: BillingActor): void {
  if (!(SUBSCRIPTION_CONFIG.billingAdminRoles as readonly string[]).includes(actor.role)) {
    throw new AppError(
      "Billing actions require admin-level access",
...
function translateStripeError(err: unknown, defaultMessage: string): AppError {
  ...
  if (stripeErr?.type === "card_error")             status = HttpStatus.BAD_REQUEST;
  if (stripeErr?.type === "invalid_request_error")  status = HttpStatus.BAD_REQUEST;
  if (stripeErr?.type === "rate_limit_error")       status = 429;
  if (stripeErr?.type === "authentication_error")   status = HttpStatus.INTERNAL;

  return new AppError(message, status, code);
}

function safeRedirectUrl(url: string | undefined, defaultPath: string): string {
  if (!url) return SUBSCRIPTION_CONFIG.clientUrl + defaultPath;
  ...
  try {
    ...
    if (parsed.hostname !== allowedHost) {
      dbLogger.warn(
        "Rejected redirect URL with foreign host: " + parsed.hostname +
...
class SubscriptionController {
  ...
  createCheckoutSession = asyncHandler(async (req: Request, res: Response, _next: NextFunction) => {
    ...
    if (!actor.email) {
      throw new AppError(
        "User email is required for billing",
        HttpStatus.BAD_REQUEST,
        "EMAIL_REQUIRED"
      );
    }
    ...
    if (!priceId) {
      throw new AppError(
        "No price configured for plan: " + validated.plan,
        HttpStatus.INTERNAL,
    ...
    if (!customerId) {
      try {
        const customer = await withTimeout(
          ...
          SUBSCRIPTION_CONFIG.stripeTimeoutMs,
          "stripe.customers.create"
        );
        ...
        dbLogger.info(
          "Stripe customer created: org=" + String(org._id) +
          " customer=" + customerId
        );
      } catch (err) {
    ...
    const idempotencyKey =
      "checkout_" + String(org._id) + "_" + validated.plan +
    ...
    try {
      ...
      dbLogger.info(
        "Checkout session created: org=" + String(org._id) +
        " plan=" + validated.plan +
        " session=" + session.id
      );
  ...
  /* ── POST /billing/cancel ── */
  cancelSubscription = asyncHandler(async (req: Request, res: Response, _next: NextFunction) => {
    const actor = requireAuth(req);
    requireBillingAdmin(actor);
    ...
    if (validated?.reason) subscription.cancelReason = validated.reason;
    ...
    dbLogger.warn(
      "Subscription cancellation: org=" + actor.organizationId +
      " actor=" + actor.userId +
      " immediate=" + immediate +
  ...
  changePlan = asyncHandler(async (req: Request, res: Response, _next: NextFunction) => {
    const actor = requireAuth(req);
    requireBillingAdmin(actor);
    ...
    try {
      ...
      const idempotencyKey =
        "plan_change_" + subscription.stripeSubscriptionId +
        "_" + validated.plan +
        "_" + Date.now().toString(36);
    ...
    dbLogger.warn(
      "Plan changed: org=" + actor.organizationId +
      " actor=" + actor.userId +
      " plan=" + validated.plan
    );
  ...
  /* ── GET /billing/history ── */
  getBillingHistory = asyncHandler(async (req: Request, res: Response, _next: NextFunction) => {
    const actor = requireAuth(req);
    requireBillingAdmin(actor);
    ...
    const limit = getNumber(
      validated.limit,
      SUBSCRIPTION_CONFIG.invoiceListLimit,
      { min: 1, max: SUBSCRIPTION_CONFIG.invoiceListMaxLimit }
    );

    const OrgModel = Organization as unknown as {
      findById: (id: string) => Promise<{ stripeCustomerId?: string } | null>;
    };

    const org = await OrgModel.findById(actor.organizationId);
    if (!org?.stripeCustomerId) {
      throw new AppError(
        "No billing history — organization has no Stripe customer",
  ...
  /* ── GET /billing/portal ── */
  createPortalSession = asyncHandler(async (req: Request, res: Response, _next: NextFunction) => {
    const actor = requireAuth(req);
    requireBillingAdmin(actor);
  ...
  getCurrentSubscription = asyncHandler(async (req: Request, res: Response, _next: NextFunction) => {
    const actor = requireAuth(req);
    requireBillingAdmin(actor);
