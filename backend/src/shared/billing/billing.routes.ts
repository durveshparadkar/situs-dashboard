...
const BILLING_ADMIN_ROLES = ["ORG_ADMIN", "SUPER_ADMIN"] as const;

const requireBillingAdmin: RequestHandler = (req, res, next) => {
  const role = String(req.user?.role ?? "").toUpperCase();
  if (!(BILLING_ADMIN_ROLES as readonly string[]).includes(role)) {
    dbLogger.warn(
      `Billing access denied: user=${req.user?.id ?? "anon"} role=${role}`
    );
    res.status(403).json({
      success: false,
      error: {
...
router.post(
  ...
  protectMw,
  requireBillingAdmin,
  authorizeMw(BILLING_MANAGE_PERM),
