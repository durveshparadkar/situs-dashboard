...
import { getPlan, type PlanTier } from "../billing/plan.js";
 ...
 */
function toLegacyLimits(tier: PlanTier): LegacyPlanLimits {
...
/**
 * @deprecated Use PlanTier from plans.ts instead.
 *
 * The union accepts the legacy SMALL_BUSINESS key plus the canonical
 * tier names so existing callers compile without changes.
 */
export type Plan = "FREE" | "SMALL" | "SMALL_BUSINESS" | "PRO" | "ENTERPRISE";
 ...
 */
export function getLegacyLimit(
  tier:     string | null | undefined,
  resource: keyof LegacyPlanLimits
): number {
  if (!tier) return 0;
