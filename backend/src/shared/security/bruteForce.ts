...
function shouldSkip(req: Request): boolean {
  ...
  if (isDev && !BRUTE_FORCE_CONFIG.enableInDev) return true;
 ...
 *   }
 */
export const bruteForceGuard: RequestHandler = async (
  req:  Request,
  res:  Response,
  next: NextFunction
) => {
 ...
 */
export async function recordFailedAttempt(req: Request): Promise<{
  blocked:          boolean;
 ...
 */
export async function resetAttempts(req: Request): Promise<void> {
 ...
 * user submits.
 */
export async function getBruteForceStatus(req: Request): Promise<{
  blocked:         boolean;
