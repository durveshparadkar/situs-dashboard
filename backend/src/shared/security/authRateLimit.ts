...
const AUTH_LIMITER_CONFIG = {
   ...
   */
  skipPaths: ["/health", "/healthz", "/ready"] as const,
...
export const authLimiter = rateLimit({
  ...
  skip: (req: Request): boolean => {
    ...
    if ((AUTH_LIMITER_CONFIG.skipPaths as readonly string[]).includes(path)) {
      return true;
    }

    if (AUTH_LIMITER_CONFIG.skipInTest) return true;

    if (isDev && !AUTH_LIMITER_CONFIG.enableInDev) return true;
