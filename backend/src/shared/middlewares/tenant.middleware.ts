...
// ============================================================
// TYPES — augment Request with tenant context
// ============================================================

/**
 * Tenant context attached to req after the guard passes.
 * Downstream controllers read this instead of digging into req.user.
 *
 * organizationId is always a string (Mongo ObjectId in string form).
 * effectiveOrganizationId mirrors organizationId for regular users,
 * but for cross-tenant operations by bypass roles it carries the
 * overridden value while organizationId stays as the actor's home org.
 */
declare global {
 ...
 */
function getUserOrganizationId(req: Request): string {
 ...
 */
function readTenantOverride(req: Request): string {
 ...
 */
export const tenantGuard: RequestHandler = (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  ...
  if (!homeOrgId) {
    dbLogger.warn(
      "Tenant guard deny — no org: user=" + userId +
  ...
  if (!mongoose.Types.ObjectId.isValid(homeOrgId)) {
    dbLogger.error(
      "Tenant guard error — invalid org format: user=" + userId +
      " orgId=" + homeOrgId
    );
  ...
  const isBypassRole  = (TENANT_GUARD_CONFIG.bypassRoles as readonly string[]).includes(role);
  ...
  if (overrideOrgId) {
    if (!isBypassRole) {
      ...
      dbLogger.warn(
        "Tenant guard deny — cross-tenant override attempt: " +
        "user=" + userId +
        " role=" + role +
        " homeOrg=" + homeOrgId +
    ...
    if (overrideOrgId !== homeOrgId) {
      ...
      dbLogger.warn(
        "Cross-tenant access: actor=" + userId +
        " role=" + role +
        " homeOrg=" + homeOrgId +
 ...
 */
export const tenantGuardStrict: RequestHandler = (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  ...
  if (!mongoose.Types.ObjectId.isValid(homeOrgId)) {
    dbLogger.error(
      "Tenant guard (strict) — invalid org format: user=" + userId +
      " orgId=" + homeOrgId
    );
    sendAuthError(res, 403, "INVALID_ORG_FORMAT", "Invalid tenant configuration");
    return;
  ...
  if (overrideOrgId && overrideOrgId !== homeOrgId) {
    dbLogger.warn(
      "Strict tenant guard — cross-tenant override blocked: " +
      "user=" + userId +
      " role=" + getNormalizedRole(req) +
      " homeOrg=" + homeOrgId +
      " attemptedOrg=" + overrideOrgId
    ...
    sendAuthError(
      ...
      403,
      "CROSS_TENANT_DENIED",
      "Cross-tenant access not permitted for this endpoint"
    );
    return;
  }
