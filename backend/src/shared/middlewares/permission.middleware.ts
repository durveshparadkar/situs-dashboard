 ...
 *   - "any" → user must have AT LEAST ONE listed permission (default — matches original behavior)
 *   - "all" → user must have EVERY listed permission (stricter)
 */
...
export function requirePermission(
  ...
  ...rest: Array<Permission | RequirePermissionOptions>
): RequestHandler {
  ...
  let mode: PermissionCheckMode = "any";
  ...
  if (Array.isArray(first)) {
    ...
    for (const r of rest) {
      if (typeof r === "string") {
        permissions.push(r as Permission);
  ...
  return (req, res, next) => {
    ...
    if ((PERMISSION_CHECK_CONFIG.bypassRoles as readonly string[]).includes(role)) {
      next();
      return;
    }
    ...
    if (!allowedPermissions) {
      dbLogger.warn(
        "Permission check denied — unknown role: user=" + userId + " role=" + role
      );
    ...
    const allowedSet = new Set<string>(allowedPermissions as readonly string[]);
