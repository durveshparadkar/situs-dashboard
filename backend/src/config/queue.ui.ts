...
const QUEUE_UI_CONFIG = {
  ...
  */
 authMode: (process.env.QUEUE_UI_AUTH_MODE ?? "jwt").toLowerCase() as
   | "jwt"
   | "basic"
   | "both"
   | "none",
...
*/
function sendForbidden(res: Response, reason: string): void {
 res.status(403).json({
   success: false,
   error: {
     code:    "FORBIDDEN",
...
*/
function buildQueueUIAuth(): RequestHandler {
 ...
 if (mode === "basic" || mode === "both") {
   if (!QUEUE_UI_CONFIG.basicUsername || !QUEUE_UI_CONFIG.basicPassword) {
     dbLogger.error(
       "Queue UI auth mode includes 'basic' but QUEUE_UI_USERNAME / " +
       "QUEUE_UI_PASSWORD env vars are not set. UI will reject all requests."
     );
   }
   if (
     QUEUE_UI_CONFIG.basicPassword &&
     QUEUE_UI_CONFIG.basicPassword.length < 12
   ) {
     dbLogger.warn(
       "QUEUE_UI_PASSWORD is shorter than 12 characters. " +
 ...
 if (mode === "none") {
   dbLogger.warn(
     "Queue UI auth mode is 'none' — UI is UNAUTHENTICATED. " +
 ...
 return (req: Request, res: Response, next: NextFunction): void => {
   ...
   if (mode === "none") {
     dbLogger.warn(
       "Queue UI accessed without auth: ip=" + ip +
       " path=" + path +
       " ua=" + userAgent
     );
     next();
   ...
   if (mode === "basic" || mode === "both") {
     ...
     if (creds) {
       ...
       if (expectedUser.length > 0 && expectedPass.length > 0) {
         if (
           safeEqual(creds.username, expectedUser) &&
           safeEqual(creds.password, expectedPass)
         ) {
           basicAuthPassed = true;
         } else {
           dbLogger.warn(
             "Queue UI basic auth failed: ip=" + ip +
   ...
   if (!basicAuthPassed && (mode === "jwt" || mode === "both")) {
     ...
     if (!(QUEUE_UI_CONFIG.allowedRoles as readonly string[]).includes(role)) {
       dbLogger.warn(
         "Queue UI access denied — insufficient role: " +
       ...
       sendForbidden(
         res,
         "Queue admin requires one of: " + QUEUE_UI_CONFIG.allowedRoles.join(", ")
       );
       return;
     }
   }
...
* mountQueueUI() below.
*/
function buildBullBoardAdapter(): ExpressAdapter {
...
*   }
*/
export function mountQueueUI(app: Express): RequestHandler | null {
 if (!QUEUE_UI_CONFIG.enabled) {
   dbLogger.info(
     "Queue UI disabled (NODE_ENV=" + (process.env.NODE_ENV ?? "unknown") +
 ...
 const router  = adapter.getRouter() as unknown as RequestHandler;

 // Mount: auth middleware FIRST, then the Bull Board router
 app.use(
   QUEUE_UI_CONFIG.basePath,
   auth,
   router
 );

 dbLogger.info(
   "Queue UI mounted: path=" + QUEUE_UI_CONFIG.basePath +
...
*/
export function getQueueUIBasePath(): string {
...
*/
export function isQueueUIEnabled(): boolean {
