...
const REDIS_CONFIG = {
   ...
   */
  maxConnectionRetries: parseInt(
    process.env.REDIS_MAX_CONNECTION_RETRIES ??
 ...
 */
function getRetryDelay(times: number): number | null {
  if (times > REDIS_CONFIG.maxConnectionRetries) {
    dbLogger.error(
      "Redis exceeded max connection retries (" +
...
dbLogger.info(
  "Redis initializing: url=" + sanitizeUrl(REDIS_CONFIG.url) +
...
redis.on("connect", () => {
  connectionAttempts++;
  dbLogger.info(
    "Redis connect event: attempt=" + connectionAttempts
  );
});
...
redis.on("error", (err: Error) => {
  ...
  isReady   = false;
  dbLogger.error(
    "Redis error: " + (err?.message ?? "unknown")
  );
});
...
export async function disconnectRedis(): Promise<void> {
  ...
  try {
    ...
    dbLogger.error(
      "Redis disconnect error, forcing close: " +
...
export async function safeDel(...keys: string[]): Promise<number> {
  ...
  try {
    ...
    dbLogger.warn(
      "Redis safeDel failed: keys=" + keys.length +
      " err=" + ((err as Error)?.message ?? "unknown")
    );
    return 0;
  }
